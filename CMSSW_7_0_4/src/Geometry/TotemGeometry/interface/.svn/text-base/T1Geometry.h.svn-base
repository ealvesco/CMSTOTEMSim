...
*/
    ...
    buildGeometry();
    //std::cout << " Calling buildAlignment for Run2" <<std::endl;
    buildAlignment("Run2");
    //std::cout << " Out of buildAlignment (1)" <<std::endl;
  }
  T1Geometry(std::string filename, std::string run){
    //   float Rmin_temp[5]={148,158,167,178,187.5};
    float Rmin_temp[5]={144.5, 155.5, 164.5, 176.5, 185.5}; 
/*    float Z_temp[5][6]={{7521.4,7569.6,7526.4,7569.6,7521.4,7564.6},
      {8191.4,8239.4,8196.4,8239.6,8191.4,8234.6},
...
*/
    ...
    //std::cout << " Calling buildAlignment for " <<run <<std::endl;
    buildAlignment(run);
    //std::cout << " Out of buildAlignment (2)" <<std::endl;
  }
...
*/
    ...
    //std::cout << " Calling buildAlignment for Run2" <<std::endl;
    buildAlignment("Run2");
    //std::cout << " Out of buildAlignment (3)" <<std::endl;
  }
  ...
  void buildGeometry();
  void buildAlignment(std::string);
...
*/   
  ...
  void Align(uint32_t detid,float globalX,float globalY,float & x,float & y,float & z){
    ...
    if(lArm==0 && (laCSC == 0 || laCSC == 5 || laCSC ==4)){
      z = Survey_z[lArm][0] -21.6 - Zeta(_040.rawId()) + Zeta(detid) + Arm_Align_z[lArm];   //21.6 mm = half chamber thickness
//std::cout <<  laCSC<<  " " << ilPlane <<  " : " <<z << " "  <<Survey_z[lArm][0] << " "<< Zeta(_040.rawId())<< " " << Zeta(detid)  <<std::endl;
    }
    if(lArm==0 && (laCSC == 1 || laCSC == 2 || laCSC ==3)){
      z = Survey_z[lArm][1] -21.6 - Zeta(_041.rawId()) + Zeta(detid) + Arm_Align_z[lArm];
//std::cout << laCSC<<  " " << ilPlane <<  " : " <<  z << " "  <<Survey_z[lArm][1] << " "<< Zeta(_041.rawId())<< " " << Zeta(detid)  <<std::endl;
    }
    if(lArm==1 && (laCSC == 0 || laCSC == 5 || laCSC ==4)){
      z = Survey_z[lArm][0] +21.6 - Zeta(_140.rawId()) + Zeta(detid) + Arm_Align_z[lArm];
//std::cout <<  laCSC<<  " " << ilPlane <<  " : " <<z << " "  <<Survey_z[lArm][0] << " "<< Zeta(_140.rawId())<< " " << Zeta(detid)  <<std::endl;
    }
    if(lArm==1 && (laCSC == 1 || laCSC == 2 || laCSC ==3)){
      z = Survey_z[lArm][1] +21.6 - Zeta(_141.rawId()) + Zeta(detid) + Arm_Align_z[lArm];
//std::cout <<  laCSC<<  " " << ilPlane <<  " : " <<z << " "  <<Survey_z[lArm][1] << " "<< Zeta(_141.rawId())<< " " << Zeta(detid)  <<std::endl; 
    }
