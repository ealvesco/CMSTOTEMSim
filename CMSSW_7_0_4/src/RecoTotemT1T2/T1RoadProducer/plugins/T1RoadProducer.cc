...
T1RoadProducer::T1RoadProducer(const ParameterSet& config):theVerbosity(0),theAlignment(false),theRun("Run2"){
  // Set verbose output
  ...
  theRun = config.getParameter<string>("Run");

  theT1Geometry = new T1Geometry(thegeometryfile,theRun);
...
void T1RoadProducer::produce(Event& event, const EventSetup& setup) {
  ...
  /*   int lArm=(*T1Reco_it).t1DetId().Arm();
    int ilPlane=(*T1Reco_it).t1DetId().Plane();
    int laCSC=(*T1Reco_it).t1DetId().CSC();
  */
