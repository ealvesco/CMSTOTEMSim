...
C...Check x and particle species.
     ...
     &KFA.NE.310.AND.KFA.NE.130.and.kfa.ne.990) THEN
        WRITE(MSTU(11),5100) KF
        GOTO 9999
      ENDIF
 
C pomeron
c      if(KFA.eq.990) then
c        CALL POMPDF(X,Q2,xpompdf)
c        DO 215 IPO=-6,6
c  215     XPQ(IPO)=xpompdf(IPO)
c      return
c      endif
...
C...Anomalous photon parton distribution call.
  ...
  230     CONTINUE
       ...
       ELSEIF(KFA.EQ.990) THEN
CPOM....pomeron structure functions.
        CALL POMPDF(X,Q2,xpompdf)
        DO 215 IPO=-6,6
  215     XPQ(IPO)=xpompdf(IPO)
