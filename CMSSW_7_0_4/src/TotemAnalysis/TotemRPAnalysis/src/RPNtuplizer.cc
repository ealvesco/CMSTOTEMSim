...
using namespace totemRPAnalysis;
using totemRPAnalysis::RPNtuplizer;

const char* RPNtuplizer::name = "TotemRPAnalysis";
...
void RPNtuplizer::begin(const edm::Run& run, const edm::EventSetup& setup) {

    edm::ESHandle<BeamOpticsParams> BOParH;
    setup.get<BeamOpticsParamsRcd>().get(BOParH);
    if (!BOParH.isValid())
      throw cms::Exception("TotemRPAnalysis::beginRun") << " edm::ESHandle<BeamOpticsParams> is invalid";
...
void RPNtuplizer::fill(event_type& eventData, const edm::Event& event, const edm::EventSetup& setup){
...
void RPNtuplizer::FillEvent(const edm::Event& e, const edm::EventSetup& es)
{
  ...
  if (FindReconstrucedProtons(e))
  {
    if (left_rec_prot_fund_)
    {
      ...
      std::cout << " reconstructed_t = " << reconstructed_t << "  rec_pr_info_[0].thx " << rec_pr_info_[1].thx <<std::endl;
...
bool RPNtuplizer::FindReconstrucedProtons(const edm::Event& e)
{
  ...
  if (Verbosity_)
  	std::cout << "[TotemRPAnalysis] Finding the reconstructed protons" << std::endl;
  ...
  if (input.isValid()) {
        std::cout << "[TotemRPAnalysis] input collection " << rpReconstructedProtonCollectionLabel << " valid." << std::endl;
  	for (RPReconstructedProtonCollection::const_iterator it = input->begin(); it != input->end(); ++it) {
  		if (Verbosity_)
  			std::cout << "[TotemRPAnalysis] Reconstructed proton ksi" << it->Ksi() << "zdirection:" << it->ZDirection()
  			        << std::endl;
  		if (it->ZDirection() > 0) {
  			++right_rec_prot_fund_;
  			reconstructed_protons_[1] = &(*it);
  			if (Verbosity_)
  				std::cout << "[TotemRPAnalysis] Setting the reconstructed proton right" << std::endl;
  		}
  		if (it->ZDirection() < 0) {
  			++left_rec_prot_fund_;
  			reconstructed_protons_[0] = &(*it);
  			if (Verbosity_)
  				std::cout << "[TotemRPAnalysis] Setting the reconstructed proton left" << std::endl;
  		}
  	}
  }
  bool result = right_rec_prot_fund_ <= 1 && left_rec_prot_fund_ <= 1 && (right_rec_prot_fund_ > 0 || left_rec_prot_fund_ > 0);
  if(!result) std::cout << "[TotemRPAnalysis] reconstructed proton not found. nr. protons right= " << right_rec_prot_fund_ << ", nr. protons left= " << left_rec_prot_fund_ << std::endl;
  return result;
}
...
bool RPNtuplizer::FindSimulatedProtons(const edm::Event& e)
{
  ...
  if (input.isValid())
  {
  	...
  	for (HepMC::GenEvent::particle_const_iterator it = evt->particles_begin(); it != evt->particles_end(); ++it)
    {
      ...
      const HepMC::GenParticle *mother = g->production_vertex() && g->production_vertex()->particles_in_const_begin() != g->production_vertex()->particles_in_const_end() ? *(g->production_vertex()->particles_in_const_begin())
 : 0 ;

      int mothers_pdg_id = mother!=0 ? mother->pdg_id() : -1;
        //    std::cout<<"++++ Mproton: " << mother->pdg_id() << std::endl;

      // scanning only for particles with status == 1
      if (g_status == 1 && pdg_id == 2212 && mothers_pdg_id==2212)
      {
        const HepMC::FourVector &vtx = g->production_vertex()->position();
        const HepMC::FourVector &mom = g->momentum();
        ...
        if (mom.z() > 0)
        {
          if (!right_prim_prot_.found || ( fabs(mom.z()) > fabs(right_prim_prot_.momentum.z()) ) )
          {
            ...
            right_prim_prot_.momentum = mom; //[GeV]
            right_prim_prot_.found = true;
            std::cout<<" right_prim_prot.momentum: " <<right_prim_prot_.momentum.z()<<std::endl;

   const HepMC::GenParticle *motherr = g->production_vertex() && g->production_vertex()->particles_in_const_begin() != 
            g->production_vertex()->particles_in_const_end() ? *(g->production_vertex()->particles_in_const_begin()) : 0 ;
            std::cout<<"++++ MOTHER right_prim_proton: " << motherr->pdg_id()<<"  "<<mothers_pdg_id << std::endl;
        ...
        if (mom.z() < 0)
        {
          if (!left_prim_prot_.found || (fabs(mom.z()) > fabs(left_prim_prot_.momentum.z())) )
          {
            left_prim_prot_.vertex = vtx; //[mm]
            left_prim_prot_.momentum = mom; //[GeV]
            std::cout<<" left_prim_prot.momentum: " <<left_prim_prot_.momentum.z()<< std::endl;
            left_prim_prot_.found = true;

const HepMC::GenParticle *motherl = g->production_vertex() && g->production_vertex()->particles_in_const_begin() != 
    g->production_vertex()->particles_in_const_end() ? *(g->production_vertex()->particles_in_const_begin()) : 0 ;
            std::cout<<"++++ MOTHER left_prim_proton: " << motherl->pdg_id() << std::endl;
...
bool RPNtuplizer::FindReconstrucedProtonPair(const edm::Event& e) {
  if (Verbosity_)
  	std::cout << "[TotemRPAnalysis] Finding the reconstructed proton pair" << std::endl;
  ...
  if (!input.isValid() || input->size() != 1) {
  	if (Verbosity_)
  		std::cout << "[TotemRPAnalysis] reconstructed proton pair not found" << std::endl;
  	return false;
  } else {
  	if (Verbosity_)
  		std::cout << "[TotemRPAnalysis] reconstructed proton pair found" << std::endl;
