...
bool T2AnalyzerRaw::MettiTracciaBuona(std::vector<TrackInfo>* matricetracce,T1T2TrackCollection::const_iterator  trk, unsigned int numevent )
{
  bool flag=false;
...
//  if(verbosity)
  ...
  */
  ...
  iEvent.getByLabel("source", input);
  bool T2trigger=false;

  //std::cout<<"alive!..............................."<<std::endl;
  if(((input->triggerData.input_status_bits)&512)!=0)
    T2trigger=true;

  if(input->triggerData.input_status_bits & 1)
    TriggerBitValue2->Fill(0);
 if(input->triggerData.input_status_bits & 2)
    TriggerBitValue2->Fill(1);
 if(input->triggerData.input_status_bits & 4)
    TriggerBitValue2->Fill(2);
 if(input->triggerData.input_status_bits & 8)
    TriggerBitValue2->Fill(3);
if(input->triggerData.input_status_bits & 16)
    TriggerBitValue2->Fill(4);

if(input->triggerData.input_status_bits & 32)
    TriggerBitValue2->Fill(5);
 if(input->triggerData.input_status_bits & 64)
    TriggerBitValue2->Fill(6);
 if(input->triggerData.input_status_bits & 128)
    TriggerBitValue2->Fill(7);
 if(input->triggerData.input_status_bits & 256)
    TriggerBitValue2->Fill(8);
if(input->triggerData.input_status_bits & 512)
    TriggerBitValue2->Fill(9);

if(input->triggerData.input_status_bits & 1024)
    TriggerBitValue2->Fill(10);
 if(input->triggerData.input_status_bits & 2048)
    TriggerBitValue2->Fill(11);
 if(input->triggerData.input_status_bits & 4096)
    TriggerBitValue2->Fill(12);
 if(input->triggerData.input_status_bits & 8192)
    TriggerBitValue2->Fill(13);
 if(input->triggerData.input_status_bits & 16384)
   TriggerBitValue2->Fill(14);

 if(input->triggerData.input_status_bits & 32768)
   TriggerBitValue2->Fill(15);


  TriggerBitValue->Fill(input->triggerData.input_status_bits);
  /*
  if(T2trigger==false)
    {
      std::cout<<"T2 TROGGER FALSE"<<(input->triggerData.input_status_bits)<<std::endl;
      
    }
  else
    {
      std::cout<<"T2 TROGGER true"<<(input->triggerData.input_status_bits)<<std::endl;
    }
  */
  if(T2trigger==false)
    return;
...
//std::cout<<" Event Track Size= "<<trackCollection->size()<<std::endl;
  ...
  // if(verbosity)
  //std::cout<< numevent <<" "<<MaxEvents<<std::endl;
...
void T2AnalyzerRaw::beginJob()
{
  ...
  std::cout<<" Begin Job"<<std::endl;  numevent=1;
...
*/
  ...
  for(unsigned int y=0;y<10;y++)
    ...
    TriggerBitValue= std::auto_ptr<TH1F>(new TH1F("TriggerBitValue","TriggerBitValue",34000, -0.5, 33999.5));
    TriggerBitValue2= std::auto_ptr<TH1F>(new TH1F("TriggerBitValue2","TriggerBitValue2",16, -0.5, 15.5));
...
AllHitRPadOnly->Write("");
 ...
 EFF_DRHitdet9->Write("");
   ...
   TriggerBitValue->Write("");
   TriggerBitValue2->Write("");
