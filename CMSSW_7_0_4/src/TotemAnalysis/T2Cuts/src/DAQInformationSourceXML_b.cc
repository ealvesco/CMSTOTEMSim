...
#include "TotemAnalysis/T2Cuts/interface/DAQInformationSourceXML_b.h"
...
const string DAQInformationSourceXML_b::tagArm = "arm";

// common XML Chip tags
const string DAQInformationSourceXML_b::tagChip1 = "vfat";
const string DAQInformationSourceXML_b::tagChip2 = "test_vfat";
const string DAQInformationSourceXML_b::tagTriggerVFAT1 = "trigger_vfat";

// specific RP XML tags
const string DAQInformationSourceXML_b::tagRPStation = "station";
const string DAQInformationSourceXML_b::tagRPPot = "rp_detector_set";
const string DAQInformationSourceXML_b::tagRPPlane = "rp_plane";

// specific T2 XML tags
const string DAQInformationSourceXML_b::tagT2="t2_detector_set";
const string DAQInformationSourceXML_b::tagT2detector="t2_detector";
const string DAQInformationSourceXML_b::tagT2Half="t2_half";

// specific T1 XML tags
const string DAQInformationSourceXML_b::tagT1="t1_detector_set";
const string DAQInformationSourceXML_b::tagT1Arm="t1_arm";
const string DAQInformationSourceXML_b::tagT1Plane="t1_plane";
const string DAQInformationSourceXML_b::tagT1CSC="t1_csc";

//----------------------------------------------------------------------------------------------------

DAQInformationSourceXML_b::DAQInformationSourceXML_b(std::string nameXMLfile) : xmlFileName("")
{
...
DAQInformationSourceXML_b::~DAQInformationSourceXML_b()
{ 
}

//----------------------------------------------------------------------------------------------------

boost::shared_ptr<DAQInformationT2_b> DAQInformationSourceXML_b::produceMap()
{
   
   boost::shared_ptr<DAQInformationT2_b> dataT2 (new DAQInformationT2_b());
  
  if (xmlFileName.empty()) { 
    throw cms::Exception("DAQInformationSourceXML_b::produce") << "Xml file not found" << endl;
  } else {
...
void DAQInformationSourceXML_b::ParseXML(std::string filename, const  boost::shared_ptr<DAQInformationT2_b> &dataT2)
{
#ifdef DEBUG
  printf(">> DAQInformationSourceXML_b::ParseXML(%s)\n", filename.c_str());
#endif
  ...
  try {
    ...
    throw cms::Exception("DAQInformationSourceXML_b") << "An XMLException caught with message: " << message << ".\n";
  ...
  try {
    ...
    throw cms::Exception("DAQInformationSourceXML_b") << "Cannot parse file 1`" << filename << "'." << endl;
  }

  if (!parser)
    throw cms::Exception("DAQInformationSourceXML_b") << "Cannot parse file 2`" << filename << "'." << endl;
  
  DOMDocument* xmlDoc = parser->getDocument();

  if (!xmlDoc)
    throw cms::Exception("DAQInformationSourceXML_b") << "Cannot parse file 3`" << filename << "'." << endl;
  ...
  //if (!elementRoot)
  //throw cms::Exception("DAQInformationSourceXML_b") << "File `" << filename << "' is empty." << endl;
...
FramePosition DAQInformationSourceXML_b::ChipFramePosition(xercesc::DOMNode *chipnode)
{
  FramePosition fp;
  ...
  for (unsigned int j = 0; j < attr->getLength(); j++) {    
    ...
    if (fp.SetXMLAttribute(XMLString::transcode(a->getNodeName()), XMLString::transcode(a->getNodeValue()), attributeFlag) > 1)
      throw cms::Exception("DAQInformationSourceXML_b") <<
...
bool DAQInformationSourceXML_b::RPNode(nodeType type)
{
...
bool DAQInformationSourceXML_b::T2Node(nodeType type)
{
...
bool DAQInformationSourceXML_b::T1Node(nodeType type)
{
...
bool DAQInformationSourceXML_b::CommonNode(nodeType type)
{
...
bool DAQInformationSourceXML_b::Test(DOMNode *node, const std::string &name)
{
...
DAQInformationSourceXML_b::nodeType DAQInformationSourceXML_b::GetNodeType(xercesc::DOMNode *n) {
  ...
  throw cms::Exception("DAQInformationSourceXML_b") << "Unknown tag `" << XMLString::transcode(n->getNodeName()) << "'.\n";
}
...
void DAQInformationSourceXML_b::ParseTreeT2(DOMNode *parent, nodeType parentType, unsigned int parentID, const  boost::shared_ptr<DAQInformationT2_b> &dataT2) {
...
//      position_t2Set = true;
...
#ifdef DEBUG  
	printf("DAQInformationSourceXML_b: attribcounter_t2planedescript: %i \n",attribcounter_t2planedescript);
#endif
	
	if(attribcounter_t2planedescript>=(25)) {
	  
	  edm::LogVerbatim("DAQInformationSourceXML_b")<<"T2-Plane attribute utilezed for parentID: position+info from parent ";
	  ...
	  if(testHS!=position_t2) {
	    edm::LogPrint("DAQInformationSourceXML_b") <<"T2 Xml inconsistence in pl-pls attributes and position. Only 'position attribute' taken ";
	    testHS=position_t2;		    
	  }
	  ...
	  if(ID_t2!=test_position_t2)
	    edm::LogPrint("DAQInformationSourceXML_b") <<"T2 Xml inconsistence in plane attributes and xml parents structure. Plane attributes ignored";
	  
	}
	else { 
	  //Case where arm-ht-pl-pls are NOT specified	      
	  edm::LogVerbatim("DAQInformationSourceXML_b")<<"T2 Plane have parentID: "<<parentID<<" for its VFATs. Plane Position read: "<<position_t2;
	...
	if(attribcounter_t2planedescript>=1) {
	  ...
	  if(attribcounter_t2planedescript>=5) {
	    ...
	    if (parentID!=ID_t2) {
	      edm::LogPrint("DAQInformationSourceXML_b") <<"T2 Inconsistence between plane 'id' and position from attributes. Id ignored";
	      edm::LogPrint("DAQInformationSourceXML_b") <<" T2-Parent = "<<parentID;		 			  
	    }		  				    
	  }
	  else {		   
	    toaddForParentID=ID_t2;		
	    edm::LogVerbatim("DAQInformationSourceXML_b")<<" Number of T2 plane attributes: "<< attribcounter_t2planedescript<<" T2-Plane attribute utilezed for parentID: plane 'id' only";
	  }
	} 
	else {
//	  position_t2Set = false;
	  edm::LogProblem ("DAQInformationSourceXML_b") << "T2 plane not enough specified from its attribute!";	      	      
	}
      }	   
    }
    ...
    if(idSet_t2==false){
      throw cms::Exception("DAQInformationSourceXML_b::ParseTree") << "ID_t2 not given for element `" << XMLString::transcode(n->getNodeName()) << "'" << endl;
      edm::LogProblem ("DAQInformationSourceXML_b") <<"ID_t2 not given for element `"<<XMLString::transcode(n->getNodeName()) << "'";
    }
    
    if (type == nT2Det && position_t2 > 39) {
      throw cms::Exception("DAQInformationSourceXML_b::ParseTree") << "Plane position_t2 range from 0 to 39. position_t2 = " << position_t2 << " is invalid." << endl;
      edm::LogProblem ("DAQInformationSourceXML_b") <<"Plane position_t2 range from 0 to 39. position_t2 = "<<position_t2<< " is invalid.";
      
    }
    // if (!position_t2Set && type == nChip) {
    // throw cms::Exception("DAQInformationSourceXML_b::ParseTree") << "Position not given for T2 chip in detector " << parentID << endl;
    // edm::LogProblem ("DAQInformationSourceXML_b") <<"Position not given for T2 chip in parent element-id: "<< parentID;
    // }
...
#ifdef DEBUG
	printf("DAQInformationSourceXML_b Found T2 special Vfat ChId-SLink-Symb  0x%x - %i - %i \n",ID_t2,position_t2,symId );
#endif
      }
