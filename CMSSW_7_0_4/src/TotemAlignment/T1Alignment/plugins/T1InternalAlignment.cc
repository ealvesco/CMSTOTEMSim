...
T1InternalAlignment::T1InternalAlignment(const edm::ParameterSet& iConfig):_Verbosity(0)
{
...
T1InternalAlignment::~T1InternalAlignment()
{
}
...
void
T1InternalAlignment::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup )
{
...
//  TracceRicostruite=trackCollection->size();
  ...
  for(TC_it=trackCollection->begin(); TC_it!=trackCollection->end(); TC_it++){
    ...
    if(   fabs((*TC_it).Z_at_Rmin()) < 500 && (*TC_it).Rmin()<50 && (*TC_it).ChiSquared()<40 
...
void 
T1InternalAlignment::beginJob()
{
  if(_Verbosity>0)
    cout << "T1InternalAlignment::beginJob" << endl;
...
void T1InternalAlignment::endJob() {
  if(_Verbosity>0)
    cout << "T1InternalAlignment::endJob" << endl;
...
// leggo precedente file dell'allineamento
  ...
  if(fileA!=NULL){
    while(!feof(fileA)){
      ...
      fscanf(fileA,"%u%u%u%f%f",&a,&p,&c,&x_,&y_);
...
//--------------------------------------------------
  ...
  int iret00 = fitParam(0,0,allTracks00, temp_Align_x, temp_Align_y);
  int iret01 = fitParam(0,1,allTracks01, temp_Align_x, temp_Align_y);
  int iret02 = fitParam(0,2,allTracks02, temp_Align_x, temp_Align_y);
  int iret03 = fitParam(0,3,allTracks03, temp_Align_x, temp_Align_y);
  int iret04 = fitParam(0,4,allTracks04, temp_Align_x, temp_Align_y);
  int iret05 = fitParam(0,5,allTracks05, temp_Align_x, temp_Align_y);
  int iret10 = fitParam(1,0,allTracks10, temp_Align_x, temp_Align_y);
  int iret11 = fitParam(1,1,allTracks11, temp_Align_x, temp_Align_y);
  int iret12 = fitParam(1,2,allTracks12, temp_Align_x, temp_Align_y);
  int iret13 = fitParam(1,3,allTracks13, temp_Align_x, temp_Align_y);
  int iret14 = fitParam(1,4,allTracks14, temp_Align_x, temp_Align_y);
  int iret15 = fitParam(1,5,allTracks15, temp_Align_x, temp_Align_y);
...
// salvo i risultati dell'allineamento
	...
	for(unsigned int k=0; k<6; k++){
	  ...
	  fprintf(fileB,"%hd %hd %hd", i,j,k);
	    ...
	    fprintf(file," %g %g\n",temp_Align_x[i][j][k],temp_Align_y[i][j][k]);
	    fprintf(fileB," %g %g\n",temp_Align_x[i][j][k]+Align_x[i][j][k],temp_Align_y[i][j][k]+Align_y[i][j][k]);
...
int T1InternalAlignment::fitParam(int braccio, int sestante, T1T2TrackCollection *tracce, double allineo_x[2][5][6], double allineo_y[2][5][6]){
  ...
  for(TC_it=tracce->begin(); TC_it!=tracce->end(); TC_it++){
	...
	if(fabs(z[counter])<9600 && fabs(z[counter])>9000){
	  if(z[counter]>0)
	  switch(sex){
	  case 0:
  ...
  MyFCN2 fcn(_all,_all_prev,_all_error);
...
// if not limited (vhigh <= vlow) 
  minuit->SetParameter(0,"x0",startX,0.1,0,0); 
  minuit->SetParameter(1,"y0",startY,0.1,0,0);
  minuit->SetParameter(2,"x1",startX,0.1,0,0); 
  minuit->SetParameter(3,"y1",startY,0.1,0,0);
  minuit->SetParameter(4,"x2",startX,0.1,0,0); 
  minuit->SetParameter(5,"y2",startY,0.1,0,0);
  minuit->SetParameter(6,"x3",startX,0.1,0,0); 
  minuit->SetParameter(7,"y3",startY,0.1,0,0);
  minuit->SetParameter(8,"x4",startX,0.1,0,0); 
  minuit->SetParameter(9,"y4",startY,0.1,0,0);
...
float T1InternalAlignment::Eta(float x,float y,float z){
  float xyt;
  float c=0;
  float eta2;
  xyt = sqrt(x*x + y*y);
...
float T1InternalAlignment::Phi(float x,float y){
  float c=0;
...
int T1InternalAlignment::sextant(float x, float y, float z){
    int sestante = -1;
    float fi = Phi(x,y);
//  cout << " FI " << fi << endl;
    if(z>0){
  ...
  int T1InternalAlignment::plane(float z) {
