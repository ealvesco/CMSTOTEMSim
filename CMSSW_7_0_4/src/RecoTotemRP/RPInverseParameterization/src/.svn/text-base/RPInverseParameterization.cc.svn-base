...
double RPInverseParameterization::FullVarianceCalculation(const std::vector<double>& par) const {
	...
	//!!! check the meaning of xyCorrelation, always set to zero, probably for the case of of correlated x-y scattering errors, not present in the RP case
	// check in older code if it was ever used, probably not.
...
void RPInverseParameterization::PrintFittedHitsInfo(ostream &o) {
	hits_at_rp_type::const_iterator it = hits_at_rp_.begin();
	hits_at_rp_type::const_iterator end = hits_at_rp_.end();
	for (; it != end; ++it) {
		//    double x_rp = it->second.X();
