 ...
 **/
class DumpRPGeometryModule : public edm::EDAnalyzer
{
  public:
    explicit DumpRPGeometryModule(const edm::ParameterSet&);
    ~DumpRPGeometryModule();
...
DumpRPGeometryModule::DumpRPGeometryModule(const edm::ParameterSet& ps) :
...
DumpRPGeometryModule::~DumpRPGeometryModule()
{
}

//----------------------------------------------------------------------------------------------------

void DumpRPGeometryModule::beginRun(edm::Run const&, edm::EventSetup const& iSetup)
{
}

//----------------------------------------------------------------------------------------------------

void DumpRPGeometryModule::endJob()
{
}

//----------------------------------------------------------------------------------------------------

void DumpRPGeometryModule::analyze(const edm::Event& event, const edm::EventSetup& es)
{
  ...
  throw cms::Exception("DumpRPGeometryModule") << "Unknown geometry type: `" << geometryType << "'.";
}

//----------------------------------------------------------------------------------------------------

void DumpRPGeometryModule::PrintGeometry(const TotemRPGeometry &geometry, const edm::Event& event)
{
  time_t unixTime = event.time().unixTime();
  char timeStr[50];
  strftime(timeStr, 50, "%F %T", localtime(&unixTime));
  printf(">> DumpRPGeometryModule::PrintGeometry\n\tnew %s geometry found in run=%u, event=%u, UNIX timestamp=%lu (%s)\n",
    geometryType.c_str(), event.id().run(), event.id().event(), unixTime, timeStr);

  // sensor geometry
  if (printSensorInfo)
  {
    printf("// silicon sensors:\n");
    for (TotemRPGeometry::mapType::const_iterator it = geometry.beginDet(); it != geometry.endDet(); ++it)
    {
      ...
      printf("  sensorData[%4i] = {%9.4f, %9.4f, %10.2f, %+9.6f, %+9.6f};\n", id.DetectorDecId(), x, y, z, dx, dy);
    }
  }
...
DEFINE_FWK_MODULE(DumpRPGeometryModule);
