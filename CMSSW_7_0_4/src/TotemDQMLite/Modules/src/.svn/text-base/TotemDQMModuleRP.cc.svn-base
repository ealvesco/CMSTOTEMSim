...
TotemDQMModuleRP::DiagonalPlots::DiagonalPlots(int _id) : id(_id)
{
  bool top45 = id & 2;
  bool top56 = id & 1;
  ...
  g_lrc_x_d = new TGraph(); g_lrc_x_d->SetMarkerColor(color);
  PlotManager::Register(string("RP/") + name + "/dx: left vs. right, graph", string(name) + " : dx left vs. right, graph;#Delta x_{45};#Delta x_{56}", g_lrc_x_d);
  g_lrc_x_n = new TGraph(); g_lrc_x_n->SetMarkerColor(color);
  PlotManager::Register(string("RP/") + name + "/xn: left vs. right, graph", string(name) + " : xn left vs. right, graph;x^{N}_{45};x^{N}_{56}", g_lrc_x_n);
  g_lrc_x_f = new TGraph(); g_lrc_x_f->SetMarkerColor(color);
  PlotManager::Register(string("RP/") + name + "/xf: left vs. right, graph", string(name) + " : xf left vs. right, graph;x^{F}_{45};x^{F}_{56}", g_lrc_x_f);

  g_lrc_y_d = new TGraph(); g_lrc_y_d->SetMarkerColor(color);
  PlotManager::Register(string("RP/") + name + "/dy: left vs. right, graph", string(name) + " : dy left vs. right, graph;#Delta y_{45};#Delta y_{56}", g_lrc_y_d);
  g_lrc_y_n = new TGraph(); g_lrc_y_n->SetMarkerColor(color);
  PlotManager::Register(string("RP/") + name + "/yn: left vs. right, graph", string(name) + " : yn left vs. right, graph;y^{N}_{45};y^{N}_{56}", g_lrc_y_n);
  g_lrc_y_f = new TGraph(); g_lrc_y_f->SetMarkerColor(color);
  PlotManager::Register(string("RP/") + name + "/yf: left vs. right, graph", string(name) + " : yf left vs. right, graph;y^{F}_{45};y^{F}_{56}", g_lrc_y_f);

  PlotManager::Register(string("RP/") + name + "/dx: left vs. right, histogram", string(name) + " : dx left vs. right, histogram;#Delta x_{45};#Delta x_{56}", h_lrc_x_d = new TH2D("", "", 50, 0., 0., 50, 0., 0.));
  PlotManager::Register(string("RP/") + name + "/xn: left vs. right, histogram", string(name) + " : xn left vs. right, histogram;x^{N}_{45};x^{N}_{56}", h_lrc_x_n = new TH2D("", "", 50, 0., 0., 50, 0., 0.));
  PlotManager::Register(string("RP/") + name + "/xf: left vs. right, histogram", string(name) + " : xf left vs. right, histogram;x^{F}_{45};x^{F}_{56}", h_lrc_x_f = new TH2D("", "", 50, 0., 0., 50, 0., 0.));

  PlotManager::Register(string("RP/") + name + "/dy: left vs. right, histogram", string(name) + " : dy left vs. right, histogram;#Delta y_{45};#Delta y_{56}", h_lrc_y_d = new TH2D("", "", 50, 0., 0., 50, 0., 0.));
  PlotManager::Register(string("RP/") + name + "/yn: left vs. right, histogram", string(name) + " : yn left vs. right, histogram;y^{N}_{45};y^{N}_{56}", h_lrc_y_n = new TH2D("", "", 50, 0., 0., 50, 0., 0.));
  PlotManager::Register(string("RP/") + name + "/yf: left vs. right, histogram", string(name) + " : yf left vs. right, histogram;y^{F}_{45};y^{F}_{56}", h_lrc_y_f = new TH2D("", "", 50, 0., 0., 50, 0., 0.));
}

//----------------------------------------------------------------------------------------------------

TotemDQMModuleRP::ArmPlots::ArmPlots(int _id) : id(_id)
{
...
TotemDQMModuleRP::StationPlots::StationPlots(int _id, std::set<unsigned int> planes, 
...
void TotemDQMModuleRP::StationPlots::Add(std::set<unsigned int> planes, int limit)
{
  ...
  if (limited && correlationPlotsCounter >= limit)
    printf("WARNING in TotemDQMModuleRP > Number of correlation plots for station %i has been limited to %i.\n", id, limit);
}

//----------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------

TotemDQMModuleRP::PotPlots::PotPlots() :
...
TotemDQMModuleRP::PotPlots::PotPlots(unsigned int id, const TotemRPGeometryLite *geometry)
{
...
TotemDQMModuleRP::PlanePlots::PlanePlots(unsigned int id)
{
...
TotemDQMModuleRP::TotemDQMModuleRP(const edm::ParameterSet &ps) : DQMModule(ps),
  ...
  drawRecognizedPatterns(ps.getUntrackedParameter<bool>("drawRecognizedPatterns", true)),

  geometry(new TotemRPGeometryLite) 
{
}

//----------------------------------------------------------------------------------------------------

void TotemDQMModuleRP::beginJob()
{
  // initialize diagonals
  diagonalPlots[1] = DiagonalPlots(1);  // 45 bot - 56 top
  diagonalPlots[2] = DiagonalPlots(2);  // 45 top - 45 bot

  // initialize anti-diagonals
  diagonalPlots[0] = DiagonalPlots(0);  // 45 bot - 56 bot
  diagonalPlots[3] = DiagonalPlots(3);  // 45 top - 56 top
...
TotemDQMModuleRP::~TotemDQMModuleRP()
{
  delete geometry;
}
...
void TotemDQMModuleRP::analyze(const edm::EventBase &event)
{
  //printf(">> TotemDQMModuleRP::analyze\n");
  ...
  if (!CumulativeMode())
  {
    ...
    {
      it->second.uHitsAll->Set(0);
      it->second.vHitsAll->Set(0);
      it->second.uHitsSel->Set(0);
      it->second.vHitsSel->Set(0);
      
      it->second.currentMultiTracksXY->Set(0);
      it->second.currentTrackXY->Set(0);

      // this effectively hides the blue dot, when there is no track fit
      it->second.vTrack->SetRange(-100, -100);
      it->second.uTrack->SetRange(-100, -100);
      
      // remove recognized patterns
      it->second.uView->Release(3);
      it->second.vView->Release(3);

      // 3D graphs
      it->second.currentTrackInRP->Reset();
    }

    // rp fits
    for (std::map<unsigned int, StationPlots>::iterator it = stationPlots.begin(); it != stationPlots.end(); it++)
      it->second.rpHits->Set(0);
  }
  ...
  event.getByLabel(tagStripDigi, digi);
  event.getByLabel(tagDigiCluster, digCluster);
  event.getByLabel(tagRecoHit, hits);
  event.getByLabel(tagPatternColl, patterns);
  event.getByLabel(tagTrackCandColl, trackCanColl);
  event.getByLabel(tagTrackColl, tracks);

  // "optional" data
  Handle< RPMulFittedTrackCollection > multiTracks;
  event.getByLabel(tagMultiTrackColl, multiTracks);
  ...
  // digi profile cumulative and single-event
  if (!CumulativeMode())
  {
    for (DetSetVector<RPStripDigi>::const_iterator it = digi->begin(); it != digi->end(); ++it)
    {
      ...
      planePlots[DetId].digi_profile_one_event->Reset();
  ...
  for (DetSetVector<RPStripDigi>::const_iterator it = digi->begin(); it != digi->end(); ++it)
  {
    ...
    for (DetSet<RPStripDigi>::const_iterator dit = it->begin(); dit != it->end(); ++dit)
    {
      if (CumulativeMode())
        ...
        planePlots[DetId].digi_profile_one_event->Fill(dit->GetStripNo());
      }
  }

  // cluster profile cumulative and single-event
  if (!CumulativeMode())
  {
    for (DetSetVector<RPDigCluster>::const_iterator it = digCluster->begin(); it != digCluster->end(); it++)
    {
      unsigned int DetId = TotRPDetId::RawToDecId(it->detId());
      planePlots[DetId].cluster_profile_one_event->Reset();
    }
  }
  ...
  for (DetSetVector<RPDigCluster>::const_iterator it = digCluster->begin(); it != digCluster->end(); it++)
  {
    ...
    for (DetSet<RPDigCluster>::const_iterator dit = it->begin(); dit != it->end(); ++dit)
    {
      if (CumulativeMode())
        ...
        planePlots[DetId].cluster_profile_one_event->Fill(dit->CentreStripPos());
    }
  }
  ...
  // Efficiency Plots
  // TODO
  //efficiency.Analyze(trackCanColl.product(), tracks.product(), geometry.product());
