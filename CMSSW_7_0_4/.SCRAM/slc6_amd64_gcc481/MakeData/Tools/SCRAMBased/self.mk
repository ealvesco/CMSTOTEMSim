...
DQMEcalBarrelMonitorDbModule_EX_USE := $(foreach d, self  CoralBase DQMServices/Core FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/ServiceRegistry FWCore/Concurrency RelationalAccess rootcore rootgraphics xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TrackingToolsGeomPropagators_EX_USE := $(foreach d, self  FWCore/MessageLogger FWCore/Utilities DataFormats/GeometrySurface TrackingTools/AnalyticalJacobians TrackingTools/TrajectoryState vdt,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
UtilitiesStaticAnalyzers_EX_USE := $(foreach d, self  llvm boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TrackingToolsAnalyticalJacobians_EX_USE := $(foreach d, self  TrackingTools/TrajectoryParametrization DataFormats/GeometrySurface DataFormats/Math vdt rootmath,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalCaloEcalRecAlgos_EX_USE := $(foreach d, self  clhep DataFormats/EcalRecHit FWCore/MessageLogger FWCore/ParameterSet FWCore/Framework CondFormats/ESObjects CondFormats/EcalObjects CondFormats/DataRecord vdt root rootminuit,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimTrackerCommon_EX_USE := $(foreach d, self  clhep vdt DataFormats/HepMCCandidate SimDataFormats/TrackingAnalysis SimDataFormats/TrackingHit SimDataFormats/CrossingFrame,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfacePythia8Interface_EX_USE := $(foreach d, self  FWCore/Concurrency FWCore/Framework FWCore/MessageLogger FWCore/Utilities SimDataFormats/GeneratorProducts GeneratorInterface/Core boost pythia8 hepmc clhep,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsTauReco_EX_USE := $(foreach d, self  DataFormats/Common DataFormats/RecoCandidate DataFormats/Candidate DataFormats/Math DataFormats/TrackReco DataFormats/JetReco DataFormats/ParticleFlowCandidate RecoVertex/VertexTools rootrflx rootcore,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
src_RecoEgamma_Examples_EX_USE := $(foreach d, SimDataFormats/GeneratorProducts clhep RecoEcal/EgammaCoreTools FWCore/Framework self DataFormats/JetReco DataFormats/DetId DataFormats/EgammaReco DQMServices/Components root DataFormats/EgammaCandidates MagneticField/Records FWCore/PluginManager RecoEgamma/EgammaMCTools DataFormats/Common hepmc DQMServices/Core RecoEgamma/EgammaElectronAlgos CommonTools/UtilAlgos FWCore/ServiceRegistry DataFormats/TrackingRecHit MagneticField/Engine FWCore/ParameterSet,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCorePluginManager_EX_USE := $(foreach d, self  boost tbb boost_filesystem FWCore/Utilities,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ValidationRecoTrack_EX_USE := $(foreach d, self  DQMServices/ClientConfig DQMServices/Core SimDataFormats/TrackingAnalysis DataFormats/TrackReco SimTracker/Common root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondToolsDQM_EX_USE := $(foreach d, self  CondCore/CondDB CondFormats/Common CoralBase DataFormats/Provenance FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry RelationalAccess rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoJetsJetProducers_EX_USE := $(foreach d, self  boost FWCore/Framework DataFormats/JetReco Geometry/CaloGeometry Geometry/CaloTopology Geometry/Records Geometry/CommonDetUnit SimDataFormats/CaloHit RecoJets/JetAlgorithms DataFormats/CastorReco CommonTools/Utils fastjet roottmva vdt,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfaceCore_EX_USE := $(foreach d, self  FWCore/Concurrency FWCore/ServiceRegistry FWCore/Utilities SimDataFormats/GeneratorProducts GeneratorInterface/LHEInterface boost clhep lhapdf f77compiler,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfaceExternalDecays_EX_USE := $(foreach d, self  FWCore/Concurrency FWCore/ParameterSet FWCore/Utilities GeneratorInterface/Pythia6Interface heppdt SimGeneral/HepPDTRecord tauolapp photos clhep evtgenlhc root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoBTagXMLCalibration_EX_USE := $(foreach d, self  FWCore/MessageLogger FWCore/Concurrency xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoParticleFlowPFClusterProducer_EX_USE := $(foreach d, self  DataFormats/Common DataFormats/ParticleFlowReco DataFormats/HcalDetId RecoEcal/EgammaCoreTools rootmath root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
