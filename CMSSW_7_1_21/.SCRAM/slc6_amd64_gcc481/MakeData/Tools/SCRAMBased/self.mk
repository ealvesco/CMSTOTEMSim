...
RecoEgammaPhotonIdentification_EX_USE := $(foreach d, self  FWCore/Framework FWCore/PluginManager DataFormats/HcalRecHit DataFormats/EgammaReco DataFormats/ParticleFlowCandidate RecoEcal/EgammaCoreTools DataFormats/BeamSpot Geometry/CaloGeometry RecoEgamma/EgammaIsolationAlgos,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsDTDigi_EX_USE := $(foreach d, self  DataFormats/Common DataFormats/MuonDetId DataFormats/FEDRawData boost rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TrackingToolsTrackRefitter_EX_USE := $(foreach d, self  FWCore/Framework TrackingTools/TransientTrackingRecHit TrackingTools/TransientTrack TrackingTools/TrackFitters RecoTracker/TransientTrackingRecHit RecoMuon/TransientTrackingRecHit MagneticField/Engine DataFormats/TrackerCommon Geometry/Records Geometry/TrackerNumberingBuilder clhep root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsTrackingRecHit_EX_USE := $(foreach d, self  DataFormats/Common DataFormats/GeometrySurface Geometry/CommonDetUnit FWCore/Utilities clhep rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoPixelVertexingPixelTriplets_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/PluginManager Geometry/Records Geometry/TrackerGeometryBuilder Geometry/CommonDetUnit Geometry/CommonTopologies RecoTracker/TkHitPairs MagneticField/Records MagneticField/Engine RecoTracker/TkSeedingLayers TrackingTools/TransientTrackingRecHit RecoTracker/TkSeedGenerator,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ifeq ($(strip $(DQM/PhysicsHWW)),)
DQMPhysicsHWW := self/DQM/PhysicsHWW
DQM/PhysicsHWW := DQMPhysicsHWW
DQMPhysicsHWW_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
DQMPhysicsHWW_EX_USE := $(foreach d, self  DQMServices/Core CommonTools/ParticleFlow RecoEgamma/EgammaTools RecoVertex/PrimaryVertexProducer TrackingTools/IPTools JetMETCorrections/Objects,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
DQMPhysicsHWW_EX_LIB   := DQMPhysicsHWW
ALL_EXTERNAL_PRODS += DQMPhysicsHWW
DQMPhysicsHWW_CLASS := LIBRARY
DQM/PhysicsHWW_relbigobj+=DQMPhysicsHWW
endif
...
ifeq ($(strip $(CondFormats/Serialization)),)
CondFormatsSerialization := self/CondFormats/Serialization
CondFormats/Serialization := CondFormatsSerialization
CondFormatsSerialization_EX_USE := $(foreach d, self  ,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
ALL_EXTERNAL_PRODS += CondFormatsSerialization
CondFormatsSerialization_CLASS := LIBRARY
CondFormats/Serialization_relbigobj+=CondFormatsSerialization
endif
...
DQMOfflineAlignment_EX_USE := $(foreach d, self  FWCore/Framework DQMServices/Core DQMServices/Components FWCore/PluginManager DataFormats/MuonReco RecoMuon/TrackingTools TrackingTools/TransientTrack TrackPropagation/SteppingHelixPropagator Alignment/LaserAlignment MagneticField/Engine,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
src_RecoCaloTools_Navigation_EX_USE := $(foreach d, self Geometry/CaloTopology DataFormats/EcalDetId DataFormats/HcalDetId,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoJetsJetProducers_EX_USE := $(foreach d, self  boost FWCore/Framework DataFormats/JetReco Geometry/CaloGeometry Geometry/CaloTopology Geometry/Records Geometry/CommonDetUnit SimDataFormats/CaloHit RecoJets/JetAlgorithms DataFormats/CastorReco CommonTools/Utils fastjet roottmva vdt_headers boost_serialization CondFormats/DataRecord,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsCastorObjects_EX_USE := $(foreach d, self  DataFormats/DetId DataFormats/HcalDetId FWCore/Utilities rootrflx boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMTrackerCommon_EX_USE := $(foreach d, self  DQMServices/Core FWCore/Framework CondFormats/DataRecord DataFormats/Common DataFormats/Scalers DataFormats/L1GlobalTrigger L1Trigger/GlobalTriggerAnalyzer HLTrigger/HLTcore rootgraphics,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsL1TObjects_EX_USE := $(foreach d, self  boost boost_serialization DataFormats/MuonDetId FWCore/ParameterSet FWCore/Utilities rootrflx DataFormats/L1GlobalMuonTrigger DataFormats/L1GlobalCaloTrigger DataFormats/L1GlobalTrigger DataFormats/StdDictionaries,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
JetMETCorrectionsInterpolationTables_CLASS := LIBRARY
JetMETCorrections/InterpolationTables_relbigobj+=JetMETCorrectionsInterpolationTables
endif
...
L1TriggerDTUtilities_EX_USE := $(foreach d, self  FWCore/PluginManager FWCore/ParameterSet Geometry/DTGeometry L1TriggerConfig/DTTPGConfig CondFormats/DTObjects clhep root rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryTrackerNumberingBuilderPlugins_LOC_USE := self  Geometry/TrackerNumberingBuilder CondFormats/GeometryObjects DetectorDescription/Core Geometry/Records DataFormats/DetId Geometry/CommonTopologies DataFormats/TrackerCommon
...
DQMServicesClientConfig_EX_USE := $(foreach d, self  DQMServices/Core FWCore/Concurrency xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimCalorimetryHcalSimProducers_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/Utilities SimCalorimetry/HcalSimAlgos SimGeneral/MixingModule SimDataFormats/CaloTest DataFormats/HcalCalibObjects,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SLHCUpgradeSimulationsGeometry_EX_USE := $(foreach d, self  DetectorDescription/Parser FWCore/PluginManager SimG4Core/Notification SimG4Core/Watcher SimG4Core/Geometry FWCore/ParameterSet boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalTrackerSiStripClusterizerPlugins_LOC_USE := self  RecoLocalTracker/SiStripClusterizer RecoLocalTracker/SiStripZeroSuppression
...
RecoTrackerTkSeedGeneratorPlugins_LOC_USE := self  RecoTracker/TkSeedGenerator RecoTracker/TkTrackingRegions RecoPixelVertexing/PixelTriplets RecoPixelVertexing/PixelTrackFitting RecoPixelVertexing/PixelLowPtUtilities RecoTracker/TrackProducer
...
src_Validation_RecoMET := self/Validation/RecoMET
Validation/RecoMET  := src_Validation_RecoMET
src_Validation_RecoMET_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
src_Validation_RecoMET_EX_USE := $(foreach d, FWCore/Framework self DataFormats/JetReco DataFormats/DetId Geometry/CaloGeometry DataFormats/HcalRecHit DataFormats/METReco Geometry/CaloTopology root DataFormats/EgammaCandidates FWCore/MessageLogger DQMServices/Core DataFormats/EcalDetId DataFormats/RecoCandidate DataFormats/HcalDetId DataFormats/TrackReco DataFormats/CaloTowers FWCore/ParameterSet,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfacePomwigInterface_EX_USE := $(foreach d, self  FWCore/Framework FWCore/Concurrency GeneratorInterface/Core GeneratorInterface/Herwig6Interface SimDataFormats/GeneratorProducts boost clhep heppdt f77compiler,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoBTagSoftLeptonPlugins_LOC_USE := self  DataFormats/BTauReco DataFormats/BeamSpot DataFormats/Common DataFormats/EgammaCandidates DataFormats/EgammaReco DataFormats/GeometryVector DataFormats/GsfTrackReco DataFormats/HcalRecHit DataFormats/JetReco DataFormats/Math DataFormats/MuonReco DataFormats/Provenance DataFormats/TrackReco DataFormats/VertexReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/Utilities RecoBTag/SoftLepton RecoBTau/JetTagComputer RecoCaloTools/Selectors TrackingTools/IPTools TrackingTools/Records TrackingTools/TrackAssociator TrackingTools/TransientTrack RecoEcal/EgammaCoreTools RecoEgamma/EgammaTools boost_regex rootmath
...
HLTriggerTimerPlugins_LOC_USE := self  boost_regex HLTrigger/Timer FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry DataFormats/Luminosity DataFormats/Provenance DataFormats/Scalers DataFormats/VertexReco SimDataFormats/PileupSummaryInfo DQMServices/Core RecoLuminosity/LumiProducer
...
DataFormatsGEMDigi_LCGDICTS  := x 
DataFormatsGEMDigi_EX_LIB   := DataFormatsGEMDigi
ALL_EXTERNAL_PRODS += DataFormatsGEMDigi
DataFormatsGEMDigi_CLASS := LIBRARY
DataFormats/GEMDigi_relbigobj+=DataFormatsGEMDigi
endif
...
FWCoreMessageLogger_EX_USE := $(foreach d, self  FWCore/Utilities boost tbb,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimTrackerCommon_EX_USE := $(foreach d, self  clhep vdt_headers DataFormats/HepMCCandidate SimDataFormats/TrackingAnalysis SimDataFormats/TrackingHit SimDataFormats/CrossingFrame,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryGlobalTrackingGeometryBuilderPlugin_LOC_USE := self  Geometry/MuonNumbering Geometry/Records Geometry/TrackerGeometryBuilder Geometry/CSCGeometry Geometry/DTGeometry Geometry/RPCGeometry Geometry/GEMGeometry
...
PhotosppInterface_LOC_USE := self  FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager GeneratorInterface/PhotosInterface photospp hepmc clhep
...
PhysicsToolsSelectorUtils_plugins_LOC_USE := self  PhysicsTools/SelectorUtils CommonTools/Utils DataFormats/Candidate DataFormats/TauReco DataFormats/TrackReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/Utilities
...
FWCoreSources_EX_USE := $(foreach d, self  DataFormats/Provenance FWCore/Catalog FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager FWCore/Utilities boost tbb,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalTrackerSiPixelRecHits_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet CondFormats/SiPixelObjects CalibTracker/SiPixelESProducers DataFormats/SiPixelCluster DataFormats/TrackerRecHit2D TrackingTools/TrajectoryState boost vdt_headers,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsRunInfo_EX_USE := $(foreach d, self  CondFormats/Common FWCore/Utilities CoralBase boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryMuonCommonDataPlugin_LOC_USE := self  DetectorDescription/Parser FWCore/PluginManager
...
GeneratorInterfaceSherpaInterface_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger SimDataFormats/GeneratorProducts GeneratorInterface/Core GeneratorInterface/ExternalDecays GeneratorInterface/Pythia6Interface boost clhep sherpa FWCore/Services zlib,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsTauReco_EX_USE := $(foreach d, self  DataFormats/Common DataFormats/RecoCandidate DataFormats/Candidate DataFormats/Math DataFormats/TrackReco DataFormats/JetReco DataFormats/ParticleFlowCandidate FWCore/Common rootrflx rootcore,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreConcurrency_EX_USE := $(foreach d, self  tbb xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
FWCoreConcurrency_EX_LIB   := FWCoreConcurrency
ALL_EXTERNAL_PRODS += FWCoreConcurrency
FWCoreConcurrency_CLASS := LIBRARY
FWCore/Concurrency_relbigobj+=FWCoreConcurrency
endif
...
TauAnalysisMCEmbeddingToolsPlugins_LOC_USE := self  FWCore/Framework FWCore/PluginManager FWCore/ParameterSet CommonTools/UtilAlgos CommonTools/Utils DataFormats/CSCRecHit DataFormats/DTRecHit DataFormats/EcalRecHit DataFormats/HepMCCandidate DataFormats/HcalRecHit DataFormats/METReco DataFormats/MuonDetId DataFormats/MuonReco DataFormats/ParticleFlowCandidate DataFormats/PatCandidates DataFormats/RPCRecHit PhysicsTools/JetMCUtils RecoEcal/EgammaCoreTools RecoMuon/MuonIsolation SimDataFormats/GeneratorProducts TrackingTools/TransientTrack DQMServices/Core rootmath TauAnalysis/MCEmbeddingTools
...
EventFilterUtilities_EX_USE := $(foreach d, self  FWCore/MessageLogger FWCore/ServiceRegistry EventFilter/FEDInterface curl,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ifeq ($(strip $(CondFormats/External)),)
src_CondFormats_External := self/CondFormats/External
CondFormats/External  := src_CondFormats_External
src_CondFormats_External_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
src_CondFormats_External_EX_USE := $(foreach d, boost_serialization clhep DataFormats/Provenance boost self DataFormats/EcalDetId DataFormats/DetId DataFormats/L1GlobalTrigger rootmath,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
ALL_EXTERNAL_PRODS += src_CondFormats_External
endif
...
GeneratorInterfaceCore_plugins_LOC_USE := self  FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger FWCore/Utilities SimDataFormats/GeneratorProducts GeneratorInterface/Core
...
CalibCalorimetryEcalTrivialCondModules_EX_USE := $(foreach d, self  rootcore root boost FWCore/Framework FWCore/ParameterSet CondTools/Ecal SimG4CMS/Calo CondFormats/EcalObjects CondFormats/ESObjects CondFormats/DataRecord DataFormats/EcalDetId clhep,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalCaloEcalRecAlgos_EX_USE := $(foreach d, self  clhep DataFormats/EcalRecHit FWCore/MessageLogger FWCore/ParameterSet FWCore/Framework CondFormats/ESObjects CondFormats/EcalObjects CondFormats/DataRecord vdt_headers root rootminuit,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ifeq ($(strip $(ValidationL1TAuto)),)
ValidationL1TAuto := self/src/Validation/L1T/plugins
ValidationL1TAuto_LOC_USE := self  DataFormats/L1Trigger DataFormats/HepMCCandidate DataFormats/Candidate FWCore/Framework FWCore/PluginManager FWCore/ParameterSet Validation/L1T
Validation/L1T_relbigobj+=ValidationL1TAuto
endif
...
SimCalorimetryHcalZeroSuppressionProducers_EX_USE := $(foreach d, self  CalibFormats/HcalObjects CondFormats/HcalObjects DataFormats/HcalDetId DataFormats/HcalDigi FWCore/Framework FWCore/MessageLogger FWCore/PluginManager boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreUtilities_EX_USE := $(foreach d, self  boost boost_filesystem boost_regex rootcintex rootcore rootrflx tbb libuuid,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMHcalMonitorModule_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/Utilities DQMServices/Core DQMServices/Components DQM/HcalMonitorTasks DataFormats/FEDRawData DataFormats/L1GlobalTrigger DataFormats/L1GlobalMuonTrigger CondFormats/DataRecord CondFormats/L1TObjects CondFormats/RunInfo,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimTrackerSiStripDigitizerPlugins_LOC_USE := self  SimTracker/SiStripDigitizer
...
CondFormatsPhysicsToolsObjects_EX_USE := $(foreach d, self  FWCore/MessageLogger FWCore/Utilities CondFormats/Common CondFormats/External boost sigcpp root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CalibCalorimetryEcalLaserSorting_EX_USE := $(foreach d, self  FWCore/Framework FWCore/Sources FWCore/Utilities IOPool/Streamer DataFormats/FEDRawData,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimGeneralPileupInformationPlugins_LOC_USE := self  CondFormats/CSCObjects CondFormats/SiPixelObjects FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet SimDataFormats/GeneratorProducts SimDataFormats/PileupSummaryInfo SimDataFormats/Track SimDataFormats/TrackingAnalysis SimDataFormats/TrackingHit SimDataFormats/Vertex SimTracker/Common clhep
...
CondDBPyInterface_LOC_USE := self  CondCore/Utilities boost boost_filesystem boost_python boost_regex rootcintex
...
Simulation_LOC_USE := self  SimG4CMS/Calo SimG4CMS/FP420 SimG4CMS/Forward SimG4CMS/Muon SimG4CMS/Tracker SimG4Core/Application SimG4Core/CustomPhysics SimG4Core/Generators SimG4Core/Geometry SimG4Core/HelpfulWatchers SimG4Core/MagneticField SimG4Core/Notification SimG4Core/Physics SimG4Core/PhysicsLists SimG4Core/SensitiveDetector SimRomanPot/SimFP420 Validation/HcalHits Validation/EcalHits SimDataFormats/ValidationFormats geant4static
...
DataFormatsProvenance_EX_USE := $(foreach d, self  FWCore/Utilities boost rootcore tbb FWCore/MessageLogger,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
src_JetMETCorrections_MCJet_EX_USE := $(foreach d, clhep self DataFormats/JetReco root DataFormats/Common Geometry/Records,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoTBCaloZDCTBAnalysis_EX_USE := $(foreach d, self  root FWCore/Framework FWCore/Utilities DataFormats/HcalRecHit TBDataFormats/HcalTBObjects,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimTrackerTrackerHitAssociation_LCGDICTS  := x 
...
GeometryHcalTowerAlgoPlugins_LOC_USE := self  FWCore/Framework FWCore/ParameterSet Geometry/Records Geometry/HcalTowerAlgo boost
...
GeneratorInterfaceHerwig6Interface_EX_USE := $(foreach d, self  FWCore/Concurrency SimDataFormats/GeneratorProducts GeneratorInterface/Core tauola boost clhep herwig fastjet jimmy f77compiler,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMOfflineJetMETPlugins_LOC_USE := self  DQMServices/Components DQMServices/Core FWCore/Utilities FWCore/Framework DataFormats/Common
...
ValidationEventGenerator_EX_USE := $(foreach d, self  hepmc SimDataFormats/GeneratorProducts FWCore/Framework FWCore/ParameterSet DQMServices/Core FWCore/ServiceRegistry DataFormats/Common clhep root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsHLTObjects_EX_USE := $(foreach d, self  FWCore/Utilities CondFormats/Common boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TrackingToolsAnalyticalJacobians_EX_USE := $(foreach d, self  TrackingTools/TrajectoryParametrization DataFormats/GeometrySurface DataFormats/Math vdt_headers rootmath,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
L1TriggerL1TGlobal_EX_USE := $(foreach d, self  FWCore/Framework FWCore/PluginManager FWCore/ParameterSet DataFormats/L1TCalorimeter DataFormats/L1TGlobal CondFormats/L1TObjects CondFormats/DataRecord L1Trigger/GlobalTrigger xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
AlignmentGeners_CLASS := LIBRARY
Alignment/Geners_relbigobj+=AlignmentGeners
endif
...
CondFormatsESObjects_EX_USE := $(foreach d, self  FWCore/Utilities DataFormats/EcalDetId DataFormats/Math boost boost_serialization rootmath rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimG4CMSHcalTestBeamHcalTB04Analysis_LOC_USE := self  SimG4CMS/HcalTestBeam SimG4Core/Watcher FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger FWCore/ServiceRegistry
...
SimG4CMSHcalTestBeamHcalTB02Analysis_LOC_USE := self  SimG4CMS/HcalTestBeam SimG4Core/Watcher FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger FWCore/ServiceRegistry
...
SimG4CMSHcalTestBeamHcalModule_LOC_USE := self  SimG4CMS/HcalTestBeam SimG4Core/Watcher FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger FWCore/ServiceRegistry
...
SimG4CMSHcalTestBeamHcalTB06Analysis_LOC_USE := self  SimG4CMS/HcalTestBeam SimG4Core/Watcher FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger FWCore/ServiceRegistry
...
CondFormatsDQMObjects_EX_USE := $(foreach d, self  FWCore/MessageLogger FWCore/Utilities boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryGEMGeometry_EX_USE := $(foreach d, self  Geometry/CommonDetUnit Geometry/CommonTopologies DataFormats/MuonDetId boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
GeometryGEMGeometry_EX_LIB   := GeometryGEMGeometry
ALL_EXTERNAL_PRODS += GeometryGEMGeometry
GeometryGEMGeometry_CLASS := LIBRARY
Geometry/GEMGeometry_relbigobj+=GeometryGEMGeometry
endif
...
FastSimulationParticleDecay_EX_USE := $(foreach d, self  hepmc FastSimulation/ParticlePropagator GeneratorInterface/Pythia6Interface SimDataFormats/GeneratorProducts GeneratorInterface/Core GeneratorInterface/ExternalDecays GeneratorInterface/Pythia8Interface FWCore/ServiceRegistry boost clhep pythia8 f77compiler pydata,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreCommon_EX_USE := $(foreach d, self  DataFormats/Provenance FWCore/ParameterSet FWCore/Utilities,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimCalorimetryEcalSelectiveReadoutAlgos_EX_USE := $(foreach d, self  Geometry/CaloTopology Geometry/EcalMapping DataFormats/EcalDigi DataFormats/EcalDetId boost boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsL1GlobalMuonTrigger_EX_USE := $(foreach d, self  DataFormats/Common FWCore/MessageLogger rootrflx tbb,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfaceCore_EX_USE := $(foreach d, self  FWCore/Concurrency FWCore/ServiceRegistry FWCore/Utilities SimDataFormats/GeneratorProducts GeneratorInterface/LHEInterface heppdt boost clhep lhapdf f77compiler,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
src_DQMServices_StreamerIO_EX_USE := $(foreach d, FWCore/Sources FWCore/RootAutoLibraryLoader DataFormats/Streamer boost Utilities/StorageFactory FWCore/Framework self HLTrigger/HLTanalyzers rootcintex IOPool/Stramer zlib FWCore/Catalog FWCore/PluginManager DataFormats/Provenance FWCore/Version DataFormats/Common rootcore FWCore/ServiceRegistry FWCore/ParameterSet FWCore/Utilities,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoEgammaPhotonIdentificationPlugins_LOC_USE := self  FWCore/Framework FWCore/PluginManager DataFormats/EgammaReco DataFormats/EgammaCandidates CondFormats/PhysicsToolsObjects Geometry/CaloGeometry RecoEgamma/PhotonIdentification
...
RecoBTagPerformanceDB_EX_USE := $(foreach d, self  FWCore/PluginManager FWCore/ParameterSet FWCore/Utilities FWCore/MessageLogger boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreMessageService_EX_USE := $(foreach d, self  DataFormats/Provenance FWCore/MessageLogger FWCore/ParameterSet FWCore/ServiceRegistry FWCore/Utilities boost tbb,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoJetsJetPlusTracks_plugins_LOC_USE := self  CondFormats/JetMETObjects DataFormats/Common DataFormats/EgammaCandidates DataFormats/JetReco DataFormats/Math DataFormats/MuonReco DataFormats/TrackReco FWCore/Framework FWCore/ParameterSet FWCore/PluginManager boost
...
SimCalorimetryHcalSimProducersPlugins_LOC_USE := self  SimCalorimetry/HcalSimProducers SimGeneral/MixingModule
...
SimTrackerVertexAssociation_EX_USE := $(foreach d, self  SimDataFormats/Track FWCore/ParameterSet DataFormats/TrackReco DataFormats/VertexReco DataFormats/Common DataFormats/Math FWCore/Utilities SimDataFormats/TrackingAnalysis SimTracker/Common,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalTrackerSiPixelClusterizer_EX_USE := $(foreach d, self  DataFormats/Common FWCore/ParameterSet DataFormats/SiPixelDetId DataFormats/SiPixelCluster boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CalibCalorimetryHcalTPGAlgos_EX_USE := $(foreach d, self  DataFormats/HcalDigi CalibFormats/HcalObjects Geometry/HcalTowerAlgo CalibCalorimetry/HcalAlgos DataFormats/DetId DataFormats/HcalDetId FWCore/Concurrency CondFormats/HcalObjects CalibFormats/CaloObjects CondFormats/DataRecord xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
EvtGenInterface_LOC_USE := self  FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager GeneratorInterface/EvtGenInterface SimGeneral/HepPDTRecord heppdt clhep hepmc evtgen pythia8 tauolapp photospp root
...
EvtGenLHCInterface_LOC_USE := self  FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager GeneratorInterface/EvtGenInterface GeneratorInterface/Pythia6Interface SimGeneral/HepPDTRecord heppdt clhep hepmc evtgenlhc photos
...
RecoEgammaElectronIdentification_EX_USE := $(foreach d, self  FWCore/Framework DataFormats/EgammaReco Geometry/CaloGeometry RecoEcal/EgammaCoreTools DataFormats/TrackReco MagneticField/Records MagneticField/Engine RecoEgamma/EgammaTools PhysicsTools/SelectorUtils rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoJetsJetAlgorithms_EX_USE := $(foreach d, self  fastjet ktjet DataFormats/JetReco DataFormats/Candidate FWCore/Framework SimDataFormats/Vertex SimDataFormats/Track DataFormats/HepMCCandidate FWCore/Utilities FWCore/ServiceRegistry CondFormats/JetMETObjects CondFormats/DataRecord,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoTrackerMeasurementDet_LCGDICTS  := x 
...
GeometryCaloEventSetupPlugins_LOC_USE := self  FWCore/PluginManager FWCore/Framework Geometry/Records Geometry/CaloGeometry Geometry/EcalAlgo Geometry/CaloTopology Geometry/HcalTowerAlgo CondFormats/Alignment FWCore/MessageLogger FWCore/ParameterSet zlib
Geometry/CaloEventSetup_relbigobj+=GeometryCaloEventSetupPlugins
endif
ifeq ($(strip $(JetMETCorrectionsFFTJetModulesPlugins)),)
JetMETCorrectionsFFTJetModulesPlugins := self/src/JetMETCorrections/FFTJetModules/plugins
JetMETCorrectionsFFTJetModulesPlugins_LOC_USE := self  CondCore/PluginSystem CondFormats/DataRecord CondCore/DBCommon CondCore/DBOutputService DataFormats/JetReco FWCore/Framework FWCore/PluginManager JetMETCorrections/FFTJetObjects
JetMETCorrections/FFTJetModules_relbigobj+=JetMETCorrectionsFFTJetModulesPlugins
endif
...
RecoTrackerTkNavigationPlugins_LOC_USE := self  RecoTracker/TkNavigation FWCore/PluginManager RecoTracker/Record RecoTracker/TkDetLayers MagneticField/Engine
...
RecoEgammaEgammaIsolationAlgosPlugins_LOC_USE := self  DataFormats/BeamSpot PhysicsTools/IsolationAlgos DataFormats/CaloTowers DataFormats/RecoCandidate DataFormats/EgammaCandidates DataFormats/TrackReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager Geometry/CaloGeometry Geometry/Records RecoCaloTools/Selectors CommonTools/Statistics RecoEgamma/EgammaIsolationAlgos CondFormats/EcalObjects CondFormats/DataRecord RecoLocalCalo/EcalRecAlgos DataFormats/ParticleFlowCandidate DataFormats/PatCandidates
...
RecoEgammaEgammaIsolationAlgosPlugins_isolation_cones_LOC_USE := self  DataFormats/BeamSpot PhysicsTools/IsolationAlgos DataFormats/CaloTowers DataFormats/RecoCandidate DataFormats/EgammaCandidates DataFormats/TrackReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager Geometry/CaloGeometry Geometry/Records RecoCaloTools/Selectors CommonTools/Statistics RecoEgamma/EgammaIsolationAlgos CondFormats/EcalObjects CondFormats/DataRecord RecoLocalCalo/EcalRecAlgos DataFormats/ParticleFlowCandidate DataFormats/PatCandidates
...
GeneratorInterfaceHydjet2Interface_EX_USE := $(foreach d, self  boost GeneratorInterface/Core FWCore/Framework SimDataFormats/GeneratorProducts GeneratorInterface/Pythia6Interface GeneratorInterface/PyquenInterface GeneratorInterface/ExternalDecays GeneratorInterface/HiGenCommon FWCore/Concurrency clhep f77compiler DataFormats/Math DataFormats/Common DataFormats/Provenance FWCore/Common FWCore/FWLite FWCore/ParameterSet FWCore/Utilities CommonTools/UtilAlgos root rootmath rootrflx rootcore,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalCaloEcalDeadChannelRecoveryAlgos_EX_USE := $(foreach d, self  DataFormats/EcalDetId DataFormats/EcalRecHit Geometry/CaloGeometry Geometry/CaloTopology Geometry/Records root rootmath rootcore rootmlp,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMServicesStreamerIOPlugins_LOC_USE := self  FWCore/Framework FWCore/ServiceRegistry IOPool/Streamer EventFilter/Utilities DQMServices/Core
...
JetMETCorrectionsFFTJetObjects_CLASS := LIBRARY
JetMETCorrections/FFTJetObjects_relbigobj+=JetMETCorrectionsFFTJetObjects
endif
...
DQMCSCMonitorModulePlugins_LOC_USE := self  DQMServices/ClientConfig DQMServices/Components DQMServices/Core EventFilter/CSCRawToDigi FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager xerces-c
...
RecoParticleFlowPFProducerPlugins_importers_LOC_USE := self  CondFormats/DataRecord CondFormats/PhysicsToolsObjects DataFormats/CaloRecHit DataFormats/Common DataFormats/EgammaReco DataFormats/EgammaTrackReco DataFormats/GsfTrackReco DataFormats/MuonReco DataFormats/ParticleFlowCandidate DataFormats/ParticleFlowReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/Utilities RecoParticleFlow/PFClusterTools RecoParticleFlow/PFProducer RecoEcal/EgammaCoreTools Geometry/CaloTopology RecoEgamma/EgammaIsolationAlgos RecoEgamma/PhotonIdentification
...
RecoParticleFlowPFProducerPlugins_linkers_LOC_USE := self  CondFormats/DataRecord CondFormats/PhysicsToolsObjects DataFormats/CaloRecHit DataFormats/Common DataFormats/EgammaReco DataFormats/EgammaTrackReco DataFormats/GsfTrackReco DataFormats/MuonReco DataFormats/ParticleFlowCandidate DataFormats/ParticleFlowReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/Utilities RecoParticleFlow/PFClusterTools RecoParticleFlow/PFProducer RecoEcal/EgammaCoreTools Geometry/CaloTopology RecoEgamma/EgammaIsolationAlgos RecoEgamma/PhotonIdentification
...
CondFormatsLuminosity_EX_USE := $(foreach d, self  FWCore/Utilities boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsJetMETObjects_EX_USE := $(foreach d, self  DataFormats/CaloTowers FWCore/ParameterSet FWCore/Utilities boost_serialization root rootmath rootrflx clhep CondFormats/PhysicsToolsObjects,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsGEMRecHit_LCGDICTS  := x 
DataFormatsGEMRecHit_EX_LIB   := DataFormatsGEMRecHit
ALL_EXTERNAL_PRODS += DataFormatsGEMRecHit
DataFormatsGEMRecHit_CLASS := LIBRARY
DataFormats/GEMRecHit_relbigobj+=DataFormatsGEMRecHit
endif
...
CmsHiValidationPlugins_LOC_USE := self  hepmc SimDataFormats/GeneratorProducts DQMServices/Core SimGeneral/HepPDTRecord SimDataFormats/TrackingAnalysis DataFormats/SiPixelDetId DataFormats/TrackerCommon Geometry/TrackerNumberingBuilder Geometry/Records
...
EventFilterEcalRawToDigiPlugins_LOC_USE := self  EventFilter/EcalRawToDigi root DataFormats/Candidate DataFormats/EcalRecHit DataFormats/L1GlobalMuonTrigger DataFormats/L1Trigger CondFormats/L1TObjects CondFormats/DataRecord FWCore/Utilities DataFormats/Common FWCore/Framework RecoEcal/EgammaCoreTools TrackingTools/Records TrackingTools/GeomPropagators TrackingTools/TrajectoryState
...
TrackingToolsKalmanUpdatorsPlugins_LOC_USE := self  TrackingTools/KalmanUpdators TrackingTools/Records MagneticField/Engine TrackingTools/GeomPropagators
...
CondFormatsEcalObjects_EX_USE := $(foreach d, self  FWCore/Utilities CondFormats/Common CondFormats/Alignment DataFormats/StdDictionaries DataFormats/EcalDetId DataFormats/Math boost boost_serialization rootmath rootrflx clhep,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsEgammaObjects_EX_USE := $(foreach d, self  FWCore/Utilities CondFormats/Common CondFormats/PhysicsToolsObjects boost_serialization rootrflx roottmva,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsSiStripObjects_EX_USE := $(foreach d, self  CondFormats/Common DataFormats/SiStripCommon DataFormats/SiStripDetId DataFormats/FEDRawData FWCore/MessageLogger FWCore/Utilities boost boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
EventFilterFEDInterface_EX_USE := $(foreach d, self  ,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
EventFilterDTRawToDigiPlugins_LOC_USE := self  FWCore/Framework FWCore/Utilities DataFormats/FEDRawData DataFormats/DTDigi CondFormats/DTObjects CondFormats/DataRecord boost
...
TrackingToolsGeomPropagators_LOC_FLAGS_CXXFLAGS   := -O3
TrackingToolsGeomPropagators_EX_USE := $(foreach d, self  FWCore/MessageLogger FWCore/Utilities DataFormats/GeometrySurface TrackingTools/AnalyticalJacobians TrackingTools/TrajectoryState vdt_headers,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CommonAlignmentAlgorithmPlugin_LOC_USE := self  FWCore/MessageLogger FWCore/Framework FWCore/ParameterSet Alignment/CommonAlignment Alignment/CommonAlignmentAlgorithm Alignment/MuonAlignment Alignment/TrackerAlignment CondCore/DBOutputService CondFormats/DataRecord CondFormats/SiPixelObjects CondFormats/SiStripObjects FWCore/ServiceRegistry DataFormats/DetId DataFormats/SiPixelDetId DataFormats/SiStripDetId MagneticField/Engine MagneticField/Records Geometry/CommonDetUnit TrackingTools/TransientTrackingRecHit
...
EventFilterSiStripChannelChargeFilter_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet DataFormats/Common DataFormats/DetId DataFormats/SiStripDigi DataFormats/SiStripCluster DataFormats/SiStripDetId DataFormats/TrackReco Geometry/Records Geometry/TrackerGeometryBuilder,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMRCTMonitor_EX_USE := $(foreach d, self  FWCore/Framework FWCore/PluginManager FWCore/ParameterSet DQMServices/Core DataFormats/L1Trigger DataFormats/L1CaloTrigger DataFormats/L1GlobalCaloTrigger DataFormats/EcalDigi DataFormats/HcalDigi root boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsHcalObjects_EX_USE := $(foreach d, self  CondFormats/Common DataFormats/DetId DataFormats/HcalDetId Geometry/CaloTopology FWCore/Utilities FWCore/MessageLogger boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoHIHiCentralityAlgos_EX_USE := $(foreach d, self  root rootrflx FWCore/Framework FWCore/PluginManager FWCore/ParameterSet FWCore/Utilities CommonTools/UtilAlgos DataFormats/Candidate DataFormats/HcalRecHit DataFormats/Common DataFormats/HeavyIonEvent DataFormats/TrackingRecHit DataFormats/TrackerRecHit2D DataFormats/TrackReco CondFormats/HIObjects CondFormats/DataRecord CondCore/DBCommon CondCore/DBOutputService CondCore/PopCon Geometry/CaloGeometry Geometry/TrackerGeometryBuilder,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ifeq ($(strip $(EcalTimeBiasCorrectionsPyInterface)),)
EcalTimeBiasCorrectionsPyInterface := self/src/CondCore/EcalPlugins/plugins
EcalTimeBiasCorrectionsPyInterface_LOC_USE := self  CondCore/Utilities CondFormats/EcalObjects CondTools/Ecal boost boost_filesystem boost_python boost_regex
CondCore/EcalPlugins_relbigobj+=EcalTimeBiasCorrectionsPyInterface
endif
...
ifeq ($(strip $(EcalTPGLinearizationConstPyInterface)),)
EcalTPGLinearizationConstPyInterface := self/src/CondCore/EcalPlugins/plugins
EcalTPGLinearizationConstPyInterface_LOC_USE := self  CondCore/Utilities CondFormats/EcalObjects CondTools/Ecal boost boost_filesystem boost_python boost_regex
CondCore/EcalPlugins_relbigobj+=EcalTPGLinearizationConstPyInterface
endif
...
ifeq ($(strip $(ESGainPyInterface)),)
ESGainPyInterface := self/src/CondCore/EcalPlugins/plugins
ESGainPyInterface_LOC_USE := self  CondCore/Utilities CondFormats/ESObjects CondTools/Ecal boost boost_filesystem boost_python boost_regex
CondCore/EcalPlugins_relbigobj+=ESGainPyInterface
endif
...
DQMServicesExamples_EX_USE := $(foreach d, self  DQMServices/Core FWCore/Framework DataFormats/MuonReco DataFormats/EgammaCandidates DataFormats/EgammaReco DataFormats/TauReco DataFormats/JetReco DataFormats/METReco DataFormats/VertexReco DataFormats/TrackerRecHit2D HLTrigger/HLTcore Geometry/Records Geometry/TrackerGeometryBuilder JetMETCorrections/Objects RecoEcal/EgammaCoreTools DataFormats/EcalRecHit CondFormats/DataRecord CondFormats/EcalObjects RecoJets/JetProducers RecoEgamma/EgammaTools,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoBTagTrackProbability_EX_USE := $(foreach d, self  DataFormats/TrackReco FWCore/MessageLogger xerces-c boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
AlignmentCocoaDaq_EX_USE := $(foreach d, self  rootcore CondFormats/OptAlignObjects,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ValidationRecoTrack_EX_USE := $(foreach d, self  DQMServices/ClientConfig DQMServices/Core SimDataFormats/TrackingAnalysis DataFormats/TrackReco root SimTracker/TrackAssociation SimTracker/TrackerHitAssociation DQM/SiStripCommon CalibTracker/Records CommonTools/TriggerUtils RecoLocalTracker/ClusterParameterEstimator SimTracker/Records,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsCSCObjects_EX_USE := $(foreach d, self  DataFormats/MuonDetId FWCore/MessageLogger FWCore/ParameterSet rootrflx boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoPixelVertexingPixelLowPtUtilitiesPlugins_LOC_USE := self  DataFormats/VertexReco RecoPixelVertexing/PixelLowPtUtilities FWCore/Concurrency
...
CondCoreESSources_EX_USE := $(foreach d, self  FWCore/Framework FWCore/PluginManager FWCore/ParameterSet CondCore/CondDB,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalCaloHcalLaserReco_EX_USE := $(foreach d, self  DataFormats/FEDRawData FWCore/Framework FWCore/Utilities DataFormats/Common DataFormats/HcalDigi boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimG4CoreCustomPhysicsPlugins_LOC_USE := self  SimG4Core/Watcher SimG4Core/Notification FWCore/ParameterSet FWCore/ServiceRegistry geant4core clhep boost SimG4Core/CustomPhysics
...
GeometryGEMGeometryBuilder_CLASS := LIBRARY
Geometry/GEMGeometryBuilder_relbigobj+=GeometryGEMGeometryBuilder
endif
...
GeneratorInterfaceCosmicMuonGenerator_EX_USE := $(foreach d, self  boost FWCore/Framework FWCore/ServiceRegistry SimDataFormats/GeneratorProducts GeneratorInterface/Core clhep root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsStdDictionaries_LCGDICTS  := x x1 x2 x3 
...
SimCalorimetryCastorSimPlugins_LOC_USE := self  SimCalorimetry/CastorSim DataFormats/HcalDetId FWCore/PluginManager FWCore/Framework FWCore/Utilities FWCore/MessageLogger FWCore/ParameterSet SimDataFormats/CrossingFrame SimGeneral/MixingModule Geometry/HcalTowerAlgo
...
GeometryRPCGeometryBuilder_EX_USE := $(foreach d, self  Geometry/MuonNumbering Geometry/RPCGeometry DetectorDescription/Core boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoVertexAdaptiveVertexFit_EX_USE := $(foreach d, self  FWCore/ParameterSet DataFormats/GeometryCommonDetAlgo DataFormats/GeometryVector TrackingTools/TrajectoryState RecoVertex/VertexPrimitives RecoVertex/KalmanVertexFit,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoEgammaElectronIdentificationPlugins_cuts_LOC_USE := self  FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger FWCore/ServiceRegistry CondFormats/DataRecord CondFormats/EgammaObjects DataFormats/EgammaCandidates DataFormats/EcalRecHit root rootcore RecoEgamma/ElectronIdentification
...
GeometryHcalEventSetup_EX_USE := $(foreach d, self  FWCore/PluginManager FWCore/ParameterSet FWCore/Framework FWCore/MessageLogger Geometry/Records Geometry/CaloGeometry Geometry/CaloTopology Geometry/HcalTowerAlgo DataFormats/HcalDetId CondFormats/Alignment CondFormats/GeometryObjects clhep,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondCoreDBOutputService_EX_USE := $(foreach d, self  FWCore/Framework CondCore/CondDB boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfacePartonShowerVeto_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet SimDataFormats/GeneratorProducts GeneratorInterface/Pythia6Interface GeneratorInterface/AlpgenInterface GeneratorInterface/LHEInterface fastjet pythia8 f77compiler,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ifeq ($(strip $(GEMGeometryESModule)),)
GEMGeometryESModule := self/src/Geometry/GEMGeometryBuilder/plugins
GEMGeometryESModule_LOC_USE := self  Geometry/MuonNumbering Geometry/Records Geometry/GEMGeometry Geometry/GEMGeometryBuilder CondFormats/GeometryObjects DetectorDescription/Core
Geometry/GEMGeometryBuilder_relbigobj+=GEMGeometryESModule
endif
...
TrackingToolsTransientTrackingRecHit_EX_USE := $(foreach d, self  boost clhep FWCore/Utilities FWCore/Framework FWCore/MessageLogger DataFormats/Common DataFormats/TrackingRecHit DataFormats/TrackerRecHit2D FWCore/ServiceRegistry Geometry/CommonDetUnit DataFormats/GeometrySurface,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
TrackingToolsTransientTrackingRecHit_LCGDICTS  := x 
...
ifeq ($(strip $(DQM/EcalMonitorTasks)),)
DQMEcalMonitorTasks := self/DQM/EcalMonitorTasks
DQM/EcalMonitorTasks := DQMEcalMonitorTasks
DQMEcalMonitorTasks_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
DQMEcalMonitorTasks_EX_USE := $(foreach d, self  DQM/EcalCommon DQMServices/Core DataFormats/FEDRawData DataFormats/EcalDetId DataFormats/EcalDigi DataFormats/EcalRawData DataFormats/EcalRecHit DataFormats/EgammaReco DataFormats/Math DataFormats/L1GlobalTrigger DataFormats/L1GlobalMuonTrigger FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/ServiceRegistry RecoLocalCalo/EcalRecAlgos RecoEcal/EgammaCoreTools CondFormats/DataRecord CondFormats/EcalObjects CondFormats/L1TObjects Geometry/CaloTopology rootphysics,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
DQMEcalMonitorTasks_EX_LIB   := DQMEcalMonitorTasks
ALL_EXTERNAL_PRODS += DQMEcalMonitorTasks
DQMEcalMonitorTasks_CLASS := LIBRARY
DQM/EcalMonitorTasks_relbigobj+=DQMEcalMonitorTasks
endif
...
GeneratorInterfaceHiGenCommon_EX_USE := $(foreach d, self  GeneratorInterface/Core SimDataFormats/GeneratorProducts SimGeneral/HepPDTRecord FWCore/ParameterSet,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
L1TriggerL1TCalorimeter_EX_USE := $(foreach d, self  FWCore/Framework FWCore/PluginManager FWCore/ParameterSet DataFormats/L1TCalorimeter DataFormats/L1GlobalCaloTrigger CondFormats/L1TObjects CondFormats/DataRecord,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondCoreJetMETPlugins_EX_USE := $(foreach d, self  CondCore/PluginSystem CondCore/ESSources CondFormats/JetMETObjects CondFormats/DataRecord,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
ALL_EXTERNAL_PRODS += CondCoreJetMETPlugins
CondCoreJetMETPlugins_CLASS := LIBRARY
CondCore/JetMETPlugins_relbigobj+=CondCoreJetMETPlugins
endif
...
FastSimulationEgammaElectronAlgos_EX_USE := $(foreach d, self  DataFormats/BeamSpot DataFormats/Common DataFormats/DetId DataFormats/EgammaReco DataFormats/Math DataFormats/SiPixelDetId DataFormats/TrackerRecHit2D DataFormats/TrajectorySeed FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FastSimulation/ParticlePropagator FastSimulation/TrackerSetup FastSimulation/Tracking Geometry/CaloGeometry Geometry/Records Geometry/TrackerGeometryBuilder MagneticField/Records RecoEgamma/EgammaIsolationAlgos RecoTracker/Record RecoTracker/TkDetLayers RecoTracker/TkSeedGenerator SimDataFormats/Track TrackingTools/KalmanUpdators TrackingTools/MaterialEffects TrackingTools/TrajectoryState TrackingTools/TransientTrackingRecHit,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ValidationMuonRPCDigis_EX_USE := $(foreach d, self  FWCore/Framework FWCore/PluginManager DataFormats/RPCDigi Geometry/Records Geometry/RPCGeometry DQMServices/Core DataFormats/Common FWCore/ParameterSet FWCore/ServiceRegistry SimDataFormats/CrossingFrame CommonTools/UtilAlgos rootcore root FWCore/Utilities,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondCoreCondDB := self/CondCore/CondDB
CondCore/CondDB := CondCoreCondDB
CondCoreCondDB_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
CondCoreCondDB_EX_USE := $(foreach d, self  CondCore/DBCommon CondCore/IOVService CondCore/MetaDataService CondCore/TagCollection CondCore/ORA FWCore/Framework boost openssl CoralCommon CoralKernel RelationalAccess,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
CondCoreCondDB_EX_LIB   := CondCoreCondDB
ALL_EXTERNAL_PRODS += CondCoreCondDB
CondCoreCondDB_CLASS := LIBRARY
CondCore/CondDB_relbigobj+=CondCoreCondDB
endif
...
GeometryCSCGeometryBuilder_EX_USE := $(foreach d, self  DataFormats/GeometrySurface DataFormats/GeometryVector DataFormats/MuonDetId DetectorDescription/Core FWCore/MessageLogger Geometry/CSCGeometry Geometry/CommonDetUnit Geometry/MuonNumbering boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreCatalog_EX_USE := $(foreach d, self  FWCore/ServiceRegistry FWCore/Utilities FWCore/Concurrency xerces-c boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
EventFilterUtilitiesPlugins_LOC_USE := self  FWCore/MessageLogger FWCore/ServiceRegistry FWCore/Framework FWCore/Sources FWCore/Utilities IOPool/Streamer EventFilter/Utilities DataFormats/FEDRawData root boost clhep
...
ValidationL1T := self/Validation/L1T
Validation/L1T := ValidationL1T
ValidationL1T_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
ValidationL1T_EX_USE := $(foreach d, self  root DQMServices/ClientConfig DQMServices/Core DataFormats/RecoCandidate,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
ValidationL1T_EX_LIB   := ValidationL1T
ALL_EXTERNAL_PRODS += ValidationL1T
ValidationL1T_CLASS := LIBRARY
Validation/L1T_relbigobj+=ValidationL1T
endif
...
CondFormatsEcalCorrections_EX_USE := $(foreach d, self  CondFormats/Common DataFormats/EcalDetId FWCore/Utilities rootrflx boost boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMDataScoutingPlugins_LOC_USE := self  DQM/DataScouting DQMServices/Components DQMServices/Core DataFormats/TrackingRecHit DataFormats/CSCRecHit DataFormats/TrackReco
...
RecoVertexBeamSpotProducer_EX_USE := $(foreach d, self  CondCore/DBOutputService CondFormats/BeamSpotObjects DataFormats/BeamSpot DataFormats/Scalers DataFormats/Common DataFormats/SiPixelDetId DataFormats/SiStripDetId DataFormats/TrackReco DataFormats/VertexReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager FWCore/ServiceRegistry FWCore/Utilities clhep rootcore rootminuit2,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsGeometryObjects_EX_USE := $(foreach d, self  CondFormats/Common FWCore/MessageLogger DataFormats/Common DataFormats/DetId boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
src_RecoEgamma_Examples_EX_USE := $(foreach d, SimDataFormats/GeneratorProducts clhep RecoEcal/EgammaCoreTools FWCore/Framework self DataFormats/JetReco DataFormats/DetId DataFormats/EgammaReco DQMServices/Components root DataFormats/EgammaCandidates FWCore/PluginManager MagneticField/Records RecoEgamma/EgammaMCTools hepmc DataFormats/Common DQMServices/Core RecoEgamma/EgammaElectronAlgos CommonTools/UtilAlgos FWCore/ServiceRegistry DataFormats/TrackingRecHit FWCore/ParameterSet MagneticField/Engine,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryCommonTopologies_EX_USE := $(foreach d, self  FWCore/MessageLogger Utilities/General vdt_headers,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsL1TCalorimeter_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
DataFormatsL1TCalorimeter_EX_USE := $(foreach d, self  DataFormats/Candidate DataFormats/Common DataFormats/L1Trigger rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
DataFormatsL1TCalorimeter_LCGDICTS  := x 
DataFormatsL1TCalorimeter_EX_LIB   := DataFormatsL1TCalorimeter
ALL_EXTERNAL_PRODS += DataFormatsL1TCalorimeter
DataFormatsL1TCalorimeter_CLASS := LIBRARY
DataFormats/L1TCalorimeter_relbigobj+=DataFormatsL1TCalorimeter
endif
...
CondFormatsRecoMuonObjects_EX_USE := $(foreach d, self  FWCore/Utilities CondFormats/Common CondFormats/PhysicsToolsObjects boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsL1TGlobal_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
DataFormatsL1TGlobal_EX_USE := $(foreach d, self  DataFormats/Candidate DataFormats/Common DataFormats/L1Trigger rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
DataFormatsL1TGlobal_LCGDICTS  := x 
DataFormatsL1TGlobal_EX_LIB   := DataFormatsL1TGlobal
ALL_EXTERNAL_PRODS += DataFormatsL1TGlobal
DataFormatsL1TGlobal_CLASS := LIBRARY
DataFormats/L1TGlobal_relbigobj+=DataFormatsL1TGlobal
endif
...
DQMSiPixelMonitorRawData_EX_USE := $(foreach d, self  DataFormats/FEDRawData FWCore/Framework FWCore/PluginManager FWCore/ParameterSet DQMServices/Core DQM/SiPixelCommon Geometry/CommonDetUnit Geometry/Records Geometry/TrackerGeometryBuilder boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsDTObjects_EX_USE := $(foreach d, self  CondFormats/Common DataFormats/MuonDetId FWCore/Utilities CondCore/DBCommon boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CalibTrackerSiPixelESProducers_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/MessageLogger CondFormats/DataRecord CondFormats/SiPixelObjects DataFormats/Common DataFormats/SiPixelDigi CalibTracker/Records MagneticField/VolumeBasedEngine boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoEgammaEgammaPhotonProducers_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/PluginManager DataFormats/ParticleFlowCandidate DataFormats/EgammaCandidates DataFormats/TrajectorySeed DataFormats/Common DataFormats/TrackCandidate DataFormats/TrackReco DataFormats/EgammaTrackReco DataFormats/CaloRecHit DataFormats/Math Geometry/CaloGeometry Geometry/CaloTopology RecoEcal/EgammaCoreTools RecoEgamma/EgammaIsolationAlgos RecoEgamma/EgammaPhotonAlgos RecoEgamma/PhotonIdentification RecoEgamma/EgammaTools RecoTracker/MeasurementDet RecoTracker/CkfPattern RecoTracker/TrackProducer RecoVertex/KinematicFitPrimitives RecoVertex/KinematicFit DataFormats/HcalRecHit RecoCaloTools/Selectors RecoTracker/ConversionSeedGenerators clhep,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TrackingToolsKalmanUpdators_LOC_FLAGS_CXXFLAGS   := -O3
...
DQMEcalMonitorDbModule_EX_USE := $(foreach d, self  CoralBase DQMServices/Core DQM/EcalCommon FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/ServiceRegistry FWCore/Concurrency CondCore/DBOutputService OnlineDB/EcalCondDB RelationalAccess rootcore rootgraphics xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoTBCaloHcalTBTools_EX_USE := $(foreach d, self  FWCore/Framework FWCore/Utilities DataFormats/Common TBDataFormats/HcalTBObjects,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimG4CoreApplicationPlugins_LOC_USE := self  FWCore/Concurrency FWCore/Framework FWCore/ParameterSet SimDataFormats/GeneratorProducts SimDataFormats/CaloHit SimDataFormats/Track SimDataFormats/TrackingHit SimDataFormats/Vertex SimG4Core/Application geant4core hepmc
...
RecoVertexNuclearInteractionProducerPlugins_LOC_USE := self  TrackingTools/Records RecoTracker/NuclearSeedGenerator RecoVertex/NuclearInteractionProducer
...
GeneratorInterfacePythia8Interface_EX_USE := $(foreach d, self  FWCore/Concurrency FWCore/Framework FWCore/MessageLogger FWCore/Utilities SimDataFormats/GeneratorProducts GeneratorInterface/Core boost pythia8 evtgen hepmc clhep,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsOptAlignObjects_EX_USE := $(foreach d, self  CondFormats/Common DataFormats/Common FWCore/Utilities boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
BeamMonitorBx_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
DQM/BeamMonitor_relbigobj+=BeamMonitorBx
endif
ifeq ($(strip $(AlcaBeamMonitor)),)
AlcaBeamMonitor := self/src/DQM/BeamMonitor/plugins
AlcaBeamMonitor_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
...
BeamSpotProblemMonitor_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
...
AlcaBeamMonitorClient_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
...
BeamConditionsMonitor_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
...
PixelVTXMonitor_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
DQM/BeamMonitor_relbigobj+=PixelVTXMonitor
endif
ifeq ($(strip $(BeamMonitor)),)
BeamMonitor := self/src/DQM/BeamMonitor/plugins
BeamMonitor_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
DQM/BeamMonitor_relbigobj+=BeamMonitor
endif
ifeq ($(strip $(Vx3DHLTAnalyzer)),)
Vx3DHLTAnalyzer := self/src/DQM/BeamMonitor/plugins
Vx3DHLTAnalyzer_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
DQM/BeamMonitor_relbigobj+=Vx3DHLTAnalyzer
endif
ifeq ($(strip $(TKStatus)),)
TKStatus := self/src/DQM/BeamMonitor/plugins
TKStatus_LOC_USE := self  DQMServices/Components DQMServices/Core DataFormats/TrackReco FWCore/Framework DataFormats/BeamSpot CondFormats/DataRecord CondFormats/BeamSpotObjects RecoVertex/BeamSpotProducer DataFormats/VertexReco DataFormats/TrackerRecHit2D TrackingTools/PatternTools
...
GeometryDTGeometryBuilder_EX_USE := $(foreach d, self  DataFormats/GeometrySurface DataFormats/MuonDetId DetectorDescription/Core Geometry/DTGeometry Geometry/MuonNumbering clhep boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoEgammaEgammaTools_EX_USE := $(foreach d, self  FWCore/Framework DataFormats/HcalRecHit DataFormats/EgammaReco DataFormats/EgammaCandidates Geometry/CaloGeometry clhep root PhysicsTools/MVAComputer PhysicsTools/MVATrainer CondFormats/DataRecord CondFormats/EgammaObjects RecoEcal/EgammaCoreTools CondCore/DBOutputService DataFormats/ParticleFlowReco roottmva,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMServicesFwkIOPlugins_LOC_USE := self  DQMServices/Core FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry FWCore/Utilities FWCore/Catalog roothistmatrix boost_filesystem
...
CalibCalorimetryEcalLaserCorrection_EX_USE := $(foreach d, self  CondFormats/DataRecord CondFormats/EcalObjects CalibCalorimetry/EcalLaserAnalyzer DataFormats/DetId DataFormats/EcalDetId DataFormats/Provenance FWCore/MessageLogger Geometry/EcalMapping boost tbb,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoBTagXMLCalibration_EX_USE := $(foreach d, self  FWCore/MessageLogger FWCore/Concurrency xerces-c boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
PhysicsToolsMVATrainer_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry FWCore/Utilities FWCore/Concurrency CondFormats/PhysicsToolsObjects CondCore/DBOutputService PhysicsTools/MVAComputer boost rootcore roothistmatrix xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryTrackerNumberingBuilder_EX_USE := $(foreach d, self  DataFormats/GeometrySurface DetectorDescription/Core DetectorDescription/Base boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoTrackerTrackProducerPlugins_LOC_USE := self  RecoTracker/TrackProducer RecoTracker/TkDetLayers TrackingTools/TransientTrack FWCore/Utilities
...
FWCorePluginManager_EX_USE := $(foreach d, self  boost tbb rootcint boost_filesystem FWCore/Utilities,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimTrackerSiPixelDigitizerPlugins_LOC_USE := self  SimGeneral/MixingModule MagneticField/Records MagneticField/Engine FWCore/Framework FWCore/ParameterSet DataFormats/Common DataFormats/SiPixelDigi SimDataFormats/TrackingHit SimDataFormats/TrackerDigiSimLink CondFormats/SiPixelObjects Geometry/TrackerGeometryBuilder SimGeneral/NoiseGenerators SimTracker/Common CalibTracker/SiPixelESProducers gsl clhep
...
CondFormatsHIObjects_EX_USE := $(foreach d, self  CondFormats/Common FWCore/Framework boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TauSpinnerInterface_LOC_USE := self  FWCore/Concurrency FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry SimDataFormats/GeneratorProducts tauolapp lhapdf root rootmath clhep
...
TauolappInterface_LOC_USE := self  FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager GeneratorInterface/TauolaInterface SimGeneral/HepPDTRecord heppdt clhep hepmc root rootmath tauolapp
...
IORawDataHcalTBInputService_EX_USE := $(foreach d, self  FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager FWCore/Sources FWCore/Utilities DataFormats/Common DataFormats/FEDRawData boost root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoTauTagImpactParameter_EX_USE := $(foreach d, self  TrackingTools/Records FWCore/Framework FWCore/PluginManager FWCore/ParameterSet DataFormats/Common DataFormats/BTauReco DataFormats/VertexReco rootrflx DataFormats/Math boost root RecoBTag/BTagTools TrackingTools/TransientTrack rootcore rootminuit2,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DetectorDescriptionParser_EX_USE := $(foreach d, self  DetectorDescription/Algorithm DetectorDescription/Base DetectorDescription/Core DetectorDescription/ExprAlgo FWCore/ParameterSet FWCore/PluginManager FWCore/Utilities FWCore/Concurrency boost clhepheader rootmath xerces-c,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMEcalCommonPlugins_LOC_USE := self  DQM/EcalCommon
...
src_RecoBTag_ImpactParameter_EX_USE := $(foreach d, self DataFormats/BTauReco,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TopQuarkAnalysisTopPairBSM_EX_USE := $(foreach d, self  AnalysisDataFormats/TopObjects DataFormats/BeamSpot DataFormats/Candidate DataFormats/Common DataFormats/JetReco DataFormats/Math DataFormats/PatCandidates FWCore/Framework FWCore/ParameterSet PhysicsTools/HepMCCandAlgos TopQuarkAnalysis/TopTools fastjet ktjet root rootcore,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMEcalMonitorDbModulePlugins_LOC_USE := self  DQM/EcalMonitorDbModule
...
ifeq ($(strip $(LaserAlignmentProducer)),)
LaserAlignmentProducer := self/src/Alignment/LaserAlignmentSimulation/plugins
LaserAlignmentProducer_LOC_USE := self  FWCore/ParameterSet FWCore/MessageLogger clhep boost FWCore/Framework SimDataFormats/GeneratorProducts hepmc
Alignment/LaserAlignmentSimulation_relbigobj+=LaserAlignmentProducer
endif
...
ifeq ($(strip $(SiStripBackPlaneCorrectionPyInterface)),)
SiStripBackPlaneCorrectionPyInterface := self/src/CondCore/SiStripPlugins/plugins
SiStripBackPlaneCorrectionPyInterface_LOC_USE := self  CondCore/Utilities CondFormats/SiStripObjects boost boost_filesystem boost_python boost_regex
CondCore/SiStripPlugins_relbigobj+=SiStripBackPlaneCorrectionPyInterface
endif
...
GeneratorInterfacePythia6Interface_EX_USE := $(foreach d, self  boost FWCore/Framework FWCore/ServiceRegistry SimDataFormats/GeneratorProducts GeneratorInterface/Core clhep pythia6 f77compiler,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryHcalTowerAlgo_EX_USE := $(foreach d, self  Geometry/CaloGeometry Geometry/CaloTopology Geometry/HcalCommonData DataFormats/HcalDetId DataFormats/CaloTowers FWCore/MessageLogger FWCore/ParameterSet clhep boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMEcalMonitorClientPlugins_LOC_USE := self  DQM/EcalCommon DQM/EcalMonitorClient
...
ifeq ($(strip $(HiParticleTowerProducerPlugins)),)
HiParticleTowerProducerPlugins := self/src/RecoHI/HiJetAlgos/plugins
HiParticleTowerProducerPlugins_LOC_USE := self  FWCore/Framework FWCore/PluginManager RecoJets/JetProducers FWCore/ParameterSet RecoHI/HiJetAlgos DataFormats/JetReco
RecoHI/HiJetAlgos_relbigobj+=HiParticleTowerProducerPlugins
endif
ifeq ($(strip $(HiGenJetPlugins)),)
HiGenJetPlugins := self/src/RecoHI/HiJetAlgos/plugins
HiGenJetPlugins_LOC_USE := self  FWCore/Framework FWCore/PluginManager RecoJets/JetProducers FWCore/ParameterSet RecoHI/HiJetAlgos DataFormats/JetReco
RecoHI/HiJetAlgos_relbigobj+=HiGenJetPlugins
endif
ifeq ($(strip $(HiJetAlgosExtraPlugins)),)
HiJetAlgosExtraPlugins := self/src/RecoHI/HiJetAlgos/plugins
HiJetAlgosExtraPlugins_LOC_USE := self  FWCore/Framework FWCore/PluginManager RecoJets/JetProducers FWCore/ParameterSet RecoHI/HiJetAlgos DataFormats/JetReco
RecoHI/HiJetAlgos_relbigobj+=HiJetAlgosExtraPlugins
endif
...
src_GeneratorInterface_CascadeInterface_EX_USE := $(foreach d, SimDataFormats/GeneratorProducts clhep boost FWCore/Framework self pythia6 f77compiler FWCore/Concurrency cascade GeneratorInterface/Core,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfaceExhumeInterface_EX_USE := $(foreach d, self  FWCore/Framework FWCore/Concurrency GeneratorInterface/Core GeneratorInterface/Pythia6Interface SimDataFormats/GeneratorProducts boost clhep f77compiler heppdt,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoPixelVertexingPixelTripletsPlugins_LOC_FLAGS_CXXFLAGS   := -Ofast -ftree-loop-if-convert-stores -fno-math-errno
...
RecoLuminosityLumiProducerfPoly_LOC_USE := self  FWCore/Framework RecoLuminosity/LumiProducer FWCore/PluginManager
...
LumiCorrectionSource_LOC_USE := self  FWCore/Framework RecoLuminosity/LumiProducer FWCore/PluginManager xerces-c
...
DQMIntegration_EX_USE := $(foreach d, self  CoralBase DataFormats/Provenance FWCore/Catalog FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry RelationalAccess rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLuminosityLumiProducer_EX_USE := $(foreach d, self  FWCore/Framework FWCore/PluginManager FWCore/ParameterSet CoralBase RelationalAccess FWCore/ServiceRegistry FWCore/Utilities FWCore/Concurrency,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreModules_EX_USE := $(foreach d, self  DataFormats/Provenance FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/Sources boost,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondFormatsRPCObjects_EX_USE := $(foreach d, self  DataFormats/DetId DataFormats/MuonDetId FWCore/MessageLogger FWCore/Utilities boost boost_serialization rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimGeneralTrackingAnalysisPlugins_LOC_USE := self  SimGeneral/TrackingAnalysis DataFormats/TrackerCommon DataFormats/HepMCCandidate SimGeneral/MixingModule
...
CalibMuonCSCCalibrationPlugins_LOC_USE := self  CalibMuon/CSCCalibration CondFormats/CSCObjects CondFormats/DataRecord DataFormats/MuonDetId FWCore/Framework FWCore/ParameterSet FWCore/PluginManager boost
...
GeneratorInterfaceBeamHaloGenerator_EX_USE := $(foreach d, self  boost FWCore/Framework FWCore/ParameterSet SimDataFormats/GeneratorProducts GeneratorInterface/Core clhep root f77compiler,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ValidationMixing_EX_USE := $(foreach d, self  FWCore/PluginManager FWCore/Utilities DQMServices/Core SimDataFormats/CrossingFrame,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FastSimulationUtilities_EX_USE := $(foreach d, self  FWCore/ServiceRegistry FWCore/Utilities clhep root rootmath,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DQMServicesCore_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry FWCore/Utilities FWCore/Version classlib rootcintex roothistmatrix protobuf,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondCoreDBCommon_EX_USE := $(foreach d, self  boost CoralCommon CoralKernel RelationalAccess FWCore/Framework FWCore/Concurrency CondCore/ORA,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CalibrationHcalCalibAlgosPlugins_LOC_USE := self  SimDataFormats/Track CondFormats/HcalObjects DQMServices/Core DataFormats/Common DataFormats/DetId DataFormats/EcalDetId DataFormats/Math DataFormats/TrackReco FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/Utilities Geometry/Records Geometry/CaloGeometry rootphysics boost root DataFormats/CaloRecHit SimDataFormats/GeneratorProducts clhep hepmc CommonTools/UtilAlgos TrackPropagation/SteppingHelixPropagator TrackingTools/TrackAssociator
...
UtilitiesStaticAnalyzers_LOC_FLAGS_CXXFLAGS   := -w
...
UtilitiesStaticAnalyzers_CLASS := LIBRARY
Utilities/StaticAnalyzers_relbigobj+=UtilitiesStaticAnalyzers
endif
...
CondCoreRPCPlugins_LOC_FLAGS_CXXFLAGS   := -ftemplate-depth=600
...
ifeq ($(strip $(DQMEcalMonitorTasksPlugins)),)
DQMEcalMonitorTasksPlugins := self/src/DQM/EcalMonitorTasks/plugins
DQMEcalMonitorTasksPlugins_LOC_USE := self  DQM/EcalCommon DQM/EcalMonitorTasks
DQM/EcalMonitorTasks_relbigobj+=DQMEcalMonitorTasksPlugins
endif
...
TrackPropagationRungeKutta_LOC_FLAGS_CXXFLAGS   := -O3
...
CondToolsDT_EX_USE := $(foreach d, self  CondCore/CondDB CondFormats/DTObjects CondFormats/DataRecord CoralBase DataFormats/Provenance FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry CondCore/DBOutputService RelationalAccess rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreParameterSet_EX_USE := $(foreach d, self  DataFormats/Provenance FWCore/MessageLogger FWCore/PluginManager FWCore/Utilities tbb boost boost_filesystem,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondToolsDQM_EX_USE := $(foreach d, self  CondCore/CondDB CondFormats/Common CondFormats/DQMObjects CoralBase DataFormats/Provenance FWCore/Framework FWCore/ParameterSet FWCore/ServiceRegistry RelationalAccess rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
OnlineDBSiStripConfigDb_EX_USE := $(foreach d, self  DataFormats/Common DataFormats/SiStripCommon FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager FWCore/Utilities tkonlineswdb boost boost_serialization,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
FWCoreIntegration_CLASS := LIBRARY
FWCore/Integration_relbigobj+=FWCoreIntegration
endif
...
CommonToolsToolsUtilAlgos_plugins_LOC_USE := self  FWCore/Framework FWCore/ParameterSet CommonTools/UtilAlgos FWCore/ServiceRegistry DataFormats/Candidate
...
src_RecoCaloTools_Selectors := self/RecoCaloTools/Selectors
RecoCaloTools/Selectors  := src_RecoCaloTools_Selectors
src_RecoCaloTools_Selectors_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
src_RecoCaloTools_Selectors_EX_USE := $(foreach d, self Geometry/CaloGeometry,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
ALL_EXTERNAL_PRODS += src_RecoCaloTools_Selectors
endif
...
L1TriggerL1TGlobalPlugins_LOC_USE := self  FWCore/Framework FWCore/PluginManager FWCore/ParameterSet DataFormats/L1Trigger CondFormats/L1TObjects CondFormats/DataRecord CondTools/L1Trigger xerces-c L1TriggerConfig/L1GtConfigProducers L1Trigger/L1TGlobal
...
ValidationGlobalDigis_EX_USE := $(foreach d, self  FWCore/Framework DataFormats/Common FWCore/ParameterSet FWCore/MessageLogger DataFormats/EcalDigi DataFormats/EcalDetId CondFormats/EcalObjects CondFormats/DataRecord DataFormats/HcalDetId DataFormats/HcalDigi DataFormats/TrackerCommon CalibFormats/HcalObjects DataFormats/SiStripDigi DataFormats/SiStripDetId DataFormats/SiPixelDigi DataFormats/SiPixelDetId DataFormats/DTDigi DataFormats/MuonDetId DataFormats/CSCDigi DataFormats/RPCDigi Geometry/RPCGeometry Geometry/Records Geometry/TrackerGeometryBuilder SimDataFormats/ValidationFormats SimDataFormats/CrossingFrame SimDataFormats/CaloHit SimDataFormats/TrackingHit DQMServices/Core root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
TopQuarkAnalysisTopSkimming_EX_USE := $(foreach d, self  FWCore/ParameterSet DataFormats/Common,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
SimG4CoreGFlash_EX_USE := $(foreach d, self  FWCore/ParameterSet FWCore/MessageLogger FWCore/PluginManager SimG4Core/Physics SimG4Core/PhysicsLists SimGeneral/GFlash geant4core clhep boost root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
CondCoreDBCommonSQLMonitoringService_LOC_USE := self  FWCore/PluginManager FWCore/Concurrency CondCore/DBCommon rootcintex zlib
...
CondCoreDBCommonSQLiteProxy_LOC_USE := self  FWCore/PluginManager FWCore/Concurrency CondCore/DBCommon rootcintex zlib
...
CondCoreDBCommonTBufferBlobStreamingService_LOC_USE := self  FWCore/PluginManager FWCore/Concurrency CondCore/DBCommon rootcintex zlib rootrflx
...
CondCoreDBCommonRelationalAuthenticationService_LOC_USE := self  FWCore/PluginManager FWCore/Concurrency CondCore/DBCommon rootcintex zlib
...
CondCoreDBCommonXMLAuthenticationService_LOC_USE := self  FWCore/PluginManager FWCore/Concurrency CondCore/DBCommon rootcintex zlib xerces-c
...
CondCoreDBCommonOracleProxy_LOC_USE := self  FWCore/PluginManager FWCore/Concurrency CondCore/DBCommon rootcintex zlib
...
CondCoreDBCommonFrontierProxy_LOC_USE := self  FWCore/PluginManager FWCore/Concurrency CondCore/DBCommon rootcintex zlib CondCore/MetaDataService
...
IOPoolInput_EX_USE := $(foreach d, self  DataFormats/Common DataFormats/Provenance FWCore/Catalog FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/ServiceRegistry FWCore/Sources FWCore/Utilities IOPool/Common Utilities/StorageFactory boost clhep rootcore,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ifeq ($(strip $(RecoLocalMuon/GEMRecHit)),)
RecoLocalMuonGEMRecHit := self/RecoLocalMuon/GEMRecHit
RecoLocalMuon/GEMRecHit := RecoLocalMuonGEMRecHit
RecoLocalMuonGEMRecHit_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
RecoLocalMuonGEMRecHit_EX_USE := $(foreach d, self  root FWCore/Framework Geometry/Records DataFormats/TrackingRecHit DataFormats/DTRecHit DataFormats/CSCRecHit DataFormats/RPCRecHit DataFormats/RPCDigi DataFormats/GEMRecHit DataFormats/GEMDigi DataFormats/Common CondFormats/RPCObjects CondFormats/DataRecord Geometry/RPCGeometry Geometry/GEMGeometry Geometry/DTGeometry Geometry/CSCGeometry TrackingTools/TrackRefitter,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
ALL_EXTERNAL_PRODS += RecoLocalMuonGEMRecHit
RecoLocalMuonGEMRecHit_CLASS := LIBRARY
RecoLocalMuon/GEMRecHit_relbigobj+=RecoLocalMuonGEMRecHit
endif
...
DQMEcalMonitorClient_EX_USE := $(foreach d, self  DQM/EcalCommon DQMServices/Core DataFormats/EcalDetId FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/ServiceRegistry rootcore,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoEgammaEgammaHFProducersPlugins_LOC_USE := self  FWCore/Framework FWCore/ParameterSet FWCore/PluginManager Geometry/CaloGeometry DataFormats/EgammaReco Geometry/Records DataFormats/RecoCandidate SimDataFormats/GeneratorProducts root
...
CondFormatsCommon_EX_USE := $(foreach d, self  DataFormats/StdDictionaries boost boost_serialization FWCore/MessageLogger FWCore/Utilities rootrflx CondCore/ORA,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeneratorInterfaceTauolaInterface_EX_USE := $(foreach d, self  FWCore/ParameterSet FWCore/Framework FWCore/PluginManager GeneratorInterface/LHEInterface hepmc clhep heppdt,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
GeometryForwardCommonDataPlugin_LOC_USE := self  DetectorDescription/Parser FWCore/PluginManager
...
DataFormatsHLTReco_EX_USE := $(foreach d, self  FWCore/Utilities DataFormats/Common DataFormats/CLHEP DataFormats/Candidate boost rootrflx,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
ifeq ($(strip $(SimTrackerTrackerHitAssociationPlugins)),)
SimTrackerTrackerHitAssociationPlugins := self/src/SimTracker/TrackerHitAssociation/plugins
SimTrackerTrackerHitAssociationPlugins_LOC_USE := self  SimTracker/TrackerHitAssociation SimDataFormats/TrackingAnalysis
SimTracker/TrackerHitAssociation_relbigobj+=SimTrackerTrackerHitAssociationPlugins
endif
...
DQMOfflineEcalPlugins_LOC_USE := self  FWCore/Framework FWCore/ParameterSet DQMServices/Core DataFormats/L1GlobalMuonTrigger DataFormats/L1GlobalTrigger CondFormats/L1TObjects CondFormats/DataRecord DQM/EcalCommon Geometry/CaloTopology DataFormats/EcalDetId DataFormats/EcalRecHit DataFormats/EgammaReco DataFormats/EgammaCandidates RecoEgamma/EgammaTools RecoEcal/EgammaCoreTools rootphysics
...
L1TriggerGlobalTrigger_EX_LIB   := L1TriggerGlobalTrigger
...
src_DQMOffline_Ecal := self/DQMOffline/Ecal
DQMOffline/Ecal  := src_DQMOffline_Ecal
src_DQMOffline_Ecal_BuildFile    := $(RELEASETOP)/.SCRAM/$(SCRAM_ARCH)/MakeData/DirCache.mk
src_DQMOffline_Ecal_EX_USE := $(foreach d, self,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
ALL_EXTERNAL_PRODS += src_DQMOffline_Ecal
endif
...
CondCorePopCon_EX_USE := $(foreach d, self  FWCore/Framework FWCore/ParameterSet CondCore/DBOutputService,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
RecoLocalMuonDTSegmentPlugins_LOC_USE := self  FWCore/Framework FWCore/ParameterSet RecoLocalMuon/DTSegment clhep
...
SimMuonGEMDigitizer_EX_USE := $(foreach d, self  boost CondCore/DBCommon CondCore/DBOutputService CondCore/IOVService CondCore/MetaDataService CondCore/PopCon CondFormats/DataRecord CondFormats/RPCObjects CoralBase DataFormats/MuonDetId DataFormats/Provenance DataFormats/GEMDigi FWCore/Catalog FWCore/Framework FWCore/MessageLogger FWCore/ParameterSet FWCore/PluginManager FWCore/ServiceRegistry FWCore/Utilities Geometry/CommonDetUnit Geometry/Records Geometry/GEMGeometry PhysicsTools/UtilAlgos RelationalAccess root rootrflx SimDataFormats/CrossingFrame SimGeneral/HepPDTRecord Utilities/General CommonTools/UtilAlgos,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
...
DataFormatsParticleFlowReco_LCGDICTS  := x x1 
...
ValidationTrackerRecHits_EX_USE := $(foreach d, self  FWCore/Framework DQMServices/Core SimDataFormats/TrackerDigiSimLink SimTracker/TrackerHitAssociation CommonTools/TriggerUtils DataFormats/SiStripDetId DQM/SiStripCommon CalibTracker/Records CalibFormats/SiStripObjects Geometry/TrackerGeometryBuilder Geometry/Records boost root,$(if $($(d)_EX_FLAGS_NO_RECURSIVE_EXPORT),,$d))
