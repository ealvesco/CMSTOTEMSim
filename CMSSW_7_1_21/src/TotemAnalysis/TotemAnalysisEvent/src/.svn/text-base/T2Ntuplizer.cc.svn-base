...
T2Ntuplizer::T2Ntuplizer(const edm::ParameterSet& iConfig) : Ntuplizer(iConfig)
{
  ...
  */
  ...
  includePadAndStripDigi = iConfig.getParameter<bool>("T2IncludePadAndStripDigi");
...
void T2Ntuplizer::FillEvent(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  const T2PadDigiCollection* PadDigiptr = t2paddigicoll.product();

  edm::Handle<T2StripDigiCollection> t2stripdigicoll;
  iEvent.getByLabel(t2StripDigiCollectionLabel, t2stripdigicoll);
  const T2StripDigiCollection* StripDigiptr = t2stripdigicoll.product();
 ...
 t2obj.HitY.clear();
 t2obj.HitZ.clear();
 t2obj.HitTrkID.clear();
 t2obj.HitPlane.clear();
 ...
 T2GeometryUtil conv;
  ...
  if(includePadAndStripDigi)
  {
    ...
    {
      T2DetId mydet=(*itp).first;
      int detnumb= mydet.plane()*2 + mydet.planeSide() + mydet.arm()*20 + mydet.halfTelescope()*10;
